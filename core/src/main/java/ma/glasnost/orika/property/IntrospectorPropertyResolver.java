...
/**
 * IntrospectionPropertyResolver leverages JavaBeans introspector to resolve properties for provided
 * types.<br>
 *
 * @author
 */
public class IntrospectorPropertyResolver extends PropertyResolver {
  ...
  /**
   * Constructs a new IntrospectorPropertyResolver that processes transient fields (backward
   * compatibility)
   *
   ...
   * @param includePublicFields whether properties for public fields should be processed as
   *     properties
   * @param includeTransientFields whether properties (getters) annotated with <code>
   *     java.beans.Transient</code> should be processed
   */
   ...
   *     non-bridge method with the same name we return that if we cannot find a non-bridge method
   *     we return the bridge method back (to prevent any unintended breakage)
   */
    ...
    try {
      ...
      for (final PropertyDescriptor pd : descriptors) {
        ...
        try {
          ...
          postProcessProperty(
              property, pd, readMethod, writeMethod, type, referenceType, properties);
          ...
          throw new RuntimeException(
              "Unexpected error while trying to resolve property "
                  + referenceType.getCanonicalName()
                  + ", ["
                  + pd.getName()
                  + "]",
              e);
  ...
  /**
   * This method performs special handling to deal with deficiencies in older (pre java-7) versions
   * of the introspector, which don't properly match getters with setters in cases where only one of
   * the two came from a template method on an ancestor.
   *
   * @param property
   * @param pd
   * @param readMethod
  ...
  private Method getWriteMethod(PropertyDescriptor pd, Class<?> type, Class<?> propertyType) {
    ...
    if (writeMethod == null
        && !("Class".equals(capitalName) && Class.class.equals(parameterType))) {
      /*
       * Special handling for older versions of Introspector: if
       * one of the getter or setter is fulfilling a templated type
       * and the other is not, they may not be returned as the same
