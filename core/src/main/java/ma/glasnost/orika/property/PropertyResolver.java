...
/**
 * PropertyResolver defines the core functionality for resolving properties; specific
 * PropertyResolverStrategy implementations should extend from this class, implementing the
...
public abstract class PropertyResolver implements PropertyResolverStrategy {
  ...
  private final Map<java.lang.reflect.Type, Map<String, Property>> inlinePropertiesCache =
      new ConcurrentHashMap<java.lang.reflect.Type, Map<String, Property>>();
      ...
      synchronized (theType) {
        ...
        if (properties == null) {
          ...
          while (!types.isEmpty()) {
            ...
            List<? extends Class<? extends Object>> interfaces =
                Arrays.<Class<? extends Object>>asList(type.getInterfaces());
  ...
  /**
   * Attempt to resolve the generic type, using refereceType to resolve TypeVariables
   *
  ...
  private Type<?> resolveGenericType(
      java.lang.reflect.Type genericType, Class<?> owningType, Type<?> referenceType) {
  ...
  /**
   * Resolves the raw property type from a property descriptor; if a read method is available, use
   * it to refine the type. The results of pd.getPropertyType() are sometimes inconsistent across
   * platforms.
   *
   * @param rawType
   * @param readMethod
   ...
   * @param properties
   */
  protected Property processProperty(
      String propertyName,
      Class<?> propertyType,
      Method readMethod,
      Method writeMethod,
    ...
    if (readMethod != null || writeMethod != null) {
      ...
      if (existing == null) {
        properties.put(propertyName, property);
          ...
          .getType()
          .isAssignableFrom(
              property.getType()) /*&& !existing.getType().equals(property.getType())*/) {
        /*
  ...
  /**
   * Tests whether the specified class has type parameters either on itself or on it's super-class
   * or declared interfaces
   *
   * @param type
   * @return
   */
   ...
   */
  public Type<?> resolvePropertyType(
      Method readMethod, Class<?> rawType, Class<?> owningType, Type<?> referenceType) {

    rawType = resolveRawPropertyType(rawType, readMethod);

    Type<?> resolvedGenericType = null;
    if (referenceType.isParameterized()
        || hasTypeParameters(owningType)
        || hasTypeParameters(rawType)) {
   ...
   */
  protected void collectPublicFieldProperties(
    ...
    for (Field f : referenceType.getRawType().getFields()) {
      if (!isStatic(f.getModifiers())) {
        ...
        Type<?> genericType =
            resolveGenericType(f.getGenericType(), f.getDeclaringClass(), referenceType);
  ...
  protected boolean isNestedPropertyExpression(String expression) {
    //        return expression.replaceAll("\\:\\{" + NON_NESTED_PROPERTY_CHARACTERS + "\\}",
    // "").indexOf('.') != -1;
    return expression.replaceAll("\\:\\{" + DYNAMIC_PROPERTY_CHARACTERS + "\\}", "").indexOf('.')
        != -1;
  }
   ...
   * @return
   */
  protected NestedProperty getNestedProperty(
      java.lang.reflect.Type type, String p, Property owner) {
      ...
      while (i < ps.length) {
        try {
          ...
          throw new PropertyNotFoundException(
              "could not resolve nested property ["
                  + p
                  + "] on "
                  + type
                  + ", because "
    ...
    return new NestedProperty(
        expression.toString(), property, path.toArray(new Property[path.size()]));
  }
   ...
   * @param type
   * @param p
   * @return the Property represented by the specified element property expression
   */
  ...
  public Property getElementProperty(java.lang.reflect.Type type, String p, Property owner) {
    ...
    if (owningProperty.isMap()) {
      elementType =
          MapEntry.concreteEntryType((Type<Map<Object, Object>>) owningProperty.getType());
   ...
   */
  @SuppressWarnings("unchecked")
  public Property getIndividualElementProperty(
      java.lang.reflect.Type type, String p, Property owner) {
    ...
    if (owningProperty.isMap()) {
      ...
      elementProperty =
          new MapKeyProperty(key, elementType.getNestedType(0), elementType.getNestedType(1), null);
    } else if (owningProperty.isCollection()) {
      elementType = owningProperty.getType().getNestedType(0);
      try {
        ...
        throw new IllegalArgumentException(
            "'" + p + "' is not a valid element property for " + type);
      }
    } else if (owningProperty.isArray()) {
      elementType = owningProperty.getType().getComponentType();
      try {
   ...
   * <br>
   * Setter name can be omitted, as well as type name; if getter name is omitted, then setter name
   * must be preceded by '|', like so: <br>
   ...
   * arguments, like so: <br>
   * "name{getTheName(\"someString\")|setTheName(\"someString\", %s)}"<br>
   * If the setter is defined in this way, it should contain the string '%s' which represents the
   * value being set.
   *
   * @param type
   * @param expr
  ...
  public Property resolveInlineProperty(java.lang.reflect.Type type, String expr) {
    ...
    if (matcher.matches()) {
      ...
      throw new IllegalArgumentException(
          "'" + expr + "' is not a valid dynamic property expression");
    }
  }
  ...
  protected abstract void collectProperties(
      Class<?> type, Type<?> referenceType, Map<String, Property> properties);
}
