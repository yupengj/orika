...
/**
 * RegexPropertyResolver uses regular expressions to find properties based on patterns configured
 * for locating the read and write methods.<br>
 * <br>
 *
 * <p>The patterns provided should produce a match where group(1) returns the name of the property.
 * If you need to specify a regex that contains more than one capture group, use the constructor
 * that allows you to specify the capture group index that will return the name of the property.<br>
 * <em>Note that the name will automatically be un-capitalized, so you need not worry about defining
 * your regular expression to handle this.</em><br>
 * <br>
 * Only no-argument getter methods returning a type are considered for a read method match, and only
 * single-argument methods are considered for a write method match; the write method need not have a
 * void return type.<br>
 * <br>
 ...
 *
 * <br>
 * <strong>"read([\w]+)Property"</strong> would match a method named
 * <strong>'readMySpecialProperty'</strong>, and define the name of the corresponding property as
 * 'MySpecial', which will be automatically un-capitalized to <strong>'mySpecial'</strong>.<br>
 * <br>
 */
public class RegexPropertyResolver extends IntrospectorPropertyResolver {
   ...
   */
  public RegexPropertyResolver(
      String readMethodRegex,
      ...
      if (m.getParameterTypes().length == 0
        ...
        if (readMatcher.matches()) {
          ...
          if (name != null) {
            ...
            throw new IllegalStateException(
                "the configured readMethod regex '"
                    + readPattern
        ...
        if (writeMatcher.matches()) {
          ...
          if (name != null) {
            ...
            throw new IllegalStateException(
                "the configured writeMethod regex '"
                    + writePattern
                    + "' does not define group (1) containing the property's name");
          }
        }
