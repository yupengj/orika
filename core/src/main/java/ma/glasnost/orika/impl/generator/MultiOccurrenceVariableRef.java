    ...
    if (isArray()) {
      if (!iteratorDeclared) {
        throw new IllegalStateException(
            "Iterator must be declared in order to add elements to destination array");
      }
  ...
  public String add(VariableRef value) {
    ...
    if (isArray()) {
      ...
      throw new IllegalArgumentException(
          type() + " does not support adding elements of type " + value.type());
    }
  }
  ...
  private static class EntrySetRef extends MultiOccurrenceVariableRef {
    ...
    private static Type<?> getSourceEntryType(VariableRef sourceMap) {
      return TypeFactory.valueOf(
          Set.class, MapEntry.entryType((Type<? extends Map<Object, Object>>) sourceMap.type()));
    }
