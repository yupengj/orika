...
/**
 * The MultipleMapperWrapper wraps multiple instances of {@link Mapper} and decide dynamically
 * during Mapping which {@link Mapper} should be used.
 *
 * <p>
 *
 * @see <a
 *     href="https://github.com/orika-mapper/orika/issues/176">https://github.com/orika-mapper/orika/issues/176</a>
 */
public final class MultipleMapperWrapper extends GeneratedMapperBase {
  ...
  public MultipleMapperWrapper(
      Type<Object> typeA, Type<Object> typeB, List<Mapper<Object, Object>> mappers) {
    super();
    setAType(typeA);
    setBType(typeB);
  ...
  private Mapper<Object, Object> getMapperFor(Object a, Object b) {
    ...
    if (mapper == null) {
      ...
      sb.append(
          String.format(
              "No matching Mapper found for %s <-> %s",
              mapperKey.getAType(), mapperKey.getBType()));

      sb.append("\n");
      for (Mapper<?, ?> mapper2 : mappersRegistry) {
        sb.append(
            String.format("\t Existing Mapper: %s <-> %s", mapper2.getAType(), mapper2.getBType()));
        sb.append("\n");
        sb.append("\t");
        sb.append(
            String.format(
                "Matching-A: %s; Matching-B: %s",
