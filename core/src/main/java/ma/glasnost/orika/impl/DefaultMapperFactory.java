...
/**
 * The mapper factory is the heart of Orika, a small container where metadata are stored, it's used
 * by other components, to look up for generated mappers, converters, object factories ... etc.
 *
...
public class DefaultMapperFactory implements MapperFactory {
  ...
  protected final ConcurrentHashMap<
          Type<? extends Object>,
          ConcurrentHashMap<Type<? extends Object>, ObjectFactory<? extends Object>>>
  ...
  protected DefaultMapperFactory(MapperFactoryBuilder<?, ?> builder) {
    ...
    this.unenhanceStrategy =
        buildUnenhanceStrategy(builder.unenhanceStrategy, builder.superTypeStrategy);
    this.contextFactory = builder.mappingContextFactory;
    this.nonCyclicContextFactory =
        new NonCyclicMappingContext.Factory(this.contextFactory.getGlobalProperties());
    ...
    this.objectFactoryGenerator =
        new ObjectFactoryGenerator(
            this, builder.constructorResolverStrategy, builder.compilerStrategy);
  ...
  /**
   * Generates the UnenhanceStrategy to be used for this MapperFactory, applying the passed
   * delegateStrategy if not null.<br>
   * This allows the MapperFactory a chance to fill in the unenhance strategy with references to
   * other parts of the factory (registered mappers, converters, object factories) which may be
   * important in the "unenhancing" process.
   *
   * @param unenhanceStrategy
   * @param superTypeStrategy true if the passed UnenhanceStrategy should take full responsibility
   *     for un-enhancement; false if the default behavior should be applied as a fail-safe after
   *     consulting the passed strategy.
   * @return the resulting UnenhanceStrategy
   */
  protected UnenhanceStrategy buildUnenhanceStrategy(
      UnenhanceStrategy unenhanceStrategy, SuperTypeResolverStrategy superTypeStrategy) {
     ...
     */
    final SuperTypeResolverStrategy inaccessibleTypeStrategy =
        new DefaultSuperTypeResolverStrategy() {

          /**
           * Tests whether the specified type is accessible to both the current thread's
           * class-loader, and also to the compilerStrategy.
           *
           * @param type
          ...
          public boolean isAcceptable(Type<?> type) {
            return isTypeAccessible(type)
                && !java.lang.reflect.Proxy.class.equals(type.getRawType());
          }
        };
  ...
  /**
   * Builds the MapperFacade for this factory. Subclasses can override this method to build a custom
   * MapperFacade. Please note that this method is called from the constructor and as such
   * properties of the factory may not yet be initialized.
   *
  ...
  /**
   * Searches for a Mapper which is capable of mapping the classes identified by the provided
   * MapperKey instance
   *
   * @param mapperKey
   * @param context
   * @return the Mapper instance which is able to support the set of classes identified by the
   *     passed MapperKey
   */
  @SuppressWarnings("unchecked")
      ...
      synchronized (this) {
        ...
        if (mapper == null) {
          try {
             ...
             * valid.
             */
            if (mapperKey.getBType().isImmutable()
                && !objectFactoryRegistry.containsKey(mapperKey.getBType())) {
              throw new MappingException(
                  "No converter registered for conversion from "
                      + mapperKey.getAType()
                      + " to "
                      + mapperKey.getBType()
                      + ", nor any ObjectFactory which can generate "
                      + mapperKey.getBType()
                      + " from "
                      + mapperKey.getAType());
            }
            ...
            ClassMapBuilder<?, ?> builder =
                classMap(mapperKey.getAType(), mapperKey.getBType()).byDefault();
   ...
   */
  public boolean existsRegisteredMapper(
      Type<?> sourceType, Type<?> destinationType, boolean includeAutoGeneratedMappers) {
  ...
  protected <A, B> Mapper<A, B> getRegisteredMapper(MapperKey mapperKey) {
    return getRegisteredMapper(
        (Type<A>) mapperKey.getAType(), (Type<B>) mapperKey.getBType(), false);
  }
   ...
   */
  @SuppressWarnings("unchecked")
  private <A, B> Mapper<A, B> getRegisteredMapper(
      Type<A> typeA, Type<B> typeB, boolean includeAutoGeneratedMappers) {
    ...
    for (Mapper<?, ?> mapper : mappersRegistry) {
      ...
      } else if ((mapper.getAType().isAssignableFrom(typeA)
              && mapper.getBType().isAssignableFrom(typeB))
          || (mapper.getBType().isAssignableFrom(typeA)
              && mapper.getAType().isAssignableFrom(typeB))
          || (mapper.getAType().isAssignableFrom(typeA)
              && typeB.isAssignableFrom(mapper.getBType())
              && objFactoryBExists)
          || (mapper.getBType().isAssignableFrom(typeA)
              && typeB.isAssignableFrom(mapper.getAType())
              && objFactoryAExists)) {
    ...
    if ((objFactoryBExists || objFactoryAExists) && foundMappers.size() > 1) {
      ...
      return (Mapper<A, B>)
          new MultipleMapperWrapper(
              (Type<Object>) typeA, (Type<Object>) typeB, (List) foundMappers);
  ...
  public <D, S> void registerObjectFactory(
      ObjectFactory<D> objectFactory, Type<D> destinationType, Type<S> sourceType) {
    ...
    if (localCache == null) {
      ...
      ConcurrentHashMap<Type<? extends Object>, ObjectFactory<? extends Object>> existing =
          objectFactoryRegistry.putIfAbsent(destinationType, localCache);
      if (existing != null) {
        localCache = existing;
      }
    }
   ...
   * @param sourceType
   * @param context
   * @return an object factory capable of generating the provided destination type, if any exists.
   */
  protected <T, S> ObjectFactory<T> lookupExistingObjectFactory(
      final Type<T> destinationType, final Type<S> sourceType, final MappingContext context) {
    ...
    ConcurrentHashMap<Type<? extends Object>, ObjectFactory<? extends Object>>
        objFactoryCacheForDestType = objectFactoryRegistry.get(destinationType);
    ...
    for (Type<? extends Object> objFactoryDestType : objFactoryDestTypes) {
      if (destinationType.isAssignableFrom(objFactoryDestType)) {
        ObjectFactory<T> result =
            findObjectFactory(objectFactoryRegistry.get(objFactoryDestType), sourceType, true);
        if (result != null) {
          return result;
        }
      }
  ...
  /**
   * {@link ConcurrentHashMap#keySet()} returns with java 8 a new Class KeySetView which doesn't
   * exist in older JDKs which can produce compile issues.
   *
   * @param concurrentHashMap {@link ConcurrentHashMap}
  ...
  private <T, S> ObjectFactory<T> findObjectFactory(
      ConcurrentHashMap<Type<? extends Object>, ObjectFactory<? extends Object>>
          objFactoryCachePerSrcType,
      final Type<S> sourceType,
      boolean onlyCustomObjectFactories) {
   ...
   */
  @SuppressWarnings("unchecked")
  public <T, S> ObjectFactory<T> lookupObjectFactory(
      final Type<T> destinationType, final Type<S> sourceType, final MappingContext context) {
      ...
      synchronized (this) {
        ...
        if (useAutoMapping || !isBuilt) {
          if (constructors.length == 1 && constructors[0].getParameterTypes().length == 0) {
            ...
            try {
              result =
                  (ObjectFactory<T>) objectFactoryGenerator.build(targetType, sourceType, context);
            } catch (MappingException e) {
          ...
          if (localCache == null) {
            ...
            var existing = objectFactoryRegistry.putIfAbsent(targetType, localCache);
            if (existing != null) {
              localCache = existing;
            }
          }
  ...
  @SuppressWarnings("unchecked")
  public <S, D> Type<? extends D> lookupConcreteDestinationType(
     ...
     */
    var concreteType =
        context == null ? null : context.getConcreteClass(sourceType, destinationType);
      ...
      for (final Type<?> type : destinationSet) {
        if (destinationType.isAssignableFrom(type) && type.isConcrete()) {
          if (type.equals(destinationType)
              || existsRegisteredMapper(sourceType, type, false)
    ...
    if (destinationSet != null && !destinationSet.isEmpty()) {
      ...
      if (registeredMapper != null) {
        concreteType =
            (Type<? extends D>)
                (registeredMapper.getAType().isAssignableFrom(sourceType)
                    ? registeredMapper.getBType()
  ...
  private <S, D> boolean customObjectFactoryForDestinationExists(
   ...
   * @param type
   * @param originalType
   * @return a concrete type (if any) which has been registered for the specified abstract type
   */
  ...
  public synchronized <A, B> void registerClassMap(ClassMap<A, B> classMap) {
    classMapRegistry.put(
        new MapperKey(classMap.getAType(), classMap.getBType()),
    ...
    if (isBuilding || isBuilt) {
      ...
      try {
        ...
        GeneratedMapperBase generatedMapper =
            buildMapper(
                classMap,
                /** isAutoGenerated == * */
                isBuilding,
  ...
  public synchronized void build() {
    ...
    if (!isBuilding && !isBuilt) {
      ...
      try {
        ...
        for (Map.Entry<MapperKey, ClassMap<Object, Object>> classMapEntry :
            classMapRegistry.entrySet()) {
        ...
        Map<ClassMap<?, ?>, GeneratedMapperBase> generatedMappers =
            new HashMap<ClassMap<?, ?>, GeneratedMapperBase>();
        for (ClassMap<?, ?> classMap : classMapRegistry.values()) {
          generatedMappers.put(classMap, buildMapper(classMap, false, context));
        }

        Set<Entry<ClassMap<?, ?>, GeneratedMapperBase>> generatedMapperEntries =
            generatedMappers.entrySet();
        for (Entry<ClassMap<?, ?>, GeneratedMapperBase> generatedMapperEntry :
            generatedMapperEntries) {
          buildObjectFactories(generatedMapperEntry.getKey(), context);
          initializeUsedMappers(
              generatedMapperEntry.getValue(), generatedMapperEntry.getKey(), context);
        }
  ...
  private <S, D> void buildObjectFactories(ClassMap<S, D> classMap, MappingContext context) {
    ...
    if (classMap.getConstructorA() != null
        && lookupExistingObjectFactory(aType, TypeFactory.TYPE_OF_OBJECT, context) == null) {
    ...
    if (classMap.getConstructorB() != null
        && lookupExistingObjectFactory(bType, TypeFactory.TYPE_OF_OBJECT, context) == null) {
  ...
  private Set<MapperKey> discoverUsedMappers(MappedTypePair<?, ?> classMapBuilder) {
    ...
    for (ClassMap<?, ?> map : classMapRegistry.values()) {
      if (map.getAType().isAssignableFrom(classMapBuilder.getAType())
          && map.getBType().isAssignableFrom(classMapBuilder.getBType())) {
  ...
  private void initializeUsedMappers(
      Mapper<?, ?> mapper, ClassMap<?, ?> classMap, MappingContext context) {
    ...
    for (Mapper<Object, Object> curParrentMapper : parentMappers) {
      if (!GeneratedMapperBase.isUsedMappersInitialized(curParrentMapper)) {
        initializeUsedMappers(
            curParrentMapper,
            getClassMap(new MapperKey(curParrentMapper.getAType(), curParrentMapper.getBType())),
            context);
      }
    }
  ...
  private void collectUsedMappers(
      ClassMap<?, ?> classMap,
      Set<Mapper<Object, Object>> parentMappers,
      MapperKey parentMapperKey,
    ...
    if (parentMapper instanceof MultipleMapperWrapper) {
      ...
      Collection<Mapper<Object, Object>> fromMultipleMappers =
          multiMapperWrapper.getMappersRegistry();
  ...
  @SuppressWarnings("unchecked")
  private GeneratedMapperBase buildMapper(
      ClassMap<?, ?> classMap, boolean isAutoGenerated, MappingContext context) {
    ...
    if (classMap.getCustomizedMapper() != null) {
      final Mapper<Object, Object> customizedMapper =
          (Mapper<Object, Object>) classMap.getCustomizedMapper();
  ...
  /**
   * Registers that a mapping exists from the specified source type to the specified destination
   * type
   *
   * @param sourceType
   * @param destinationType
   */
  protected <S, D> void register(
      Type<S> sourceType, Type<D> destinationType, boolean isAutoGenerated) {
  ...
  /** @return the (initialized) ClassMapBuilderFactory configured for this mapper factory */
  protected ClassMapBuilderFactory getClassMapBuilderFactory() {
    if (!classMapBuilderFactory.isInitialized()) {
      classMapBuilderFactory.setDefaultFieldMappers(
  ...
  public <A, B> ClassMapBuilder<A, B> classMap(Type<A> aType, Type<B> bType) {
    ClassMapBuilderFactory classMapBuilderFactory =
        chainClassMapBuilderFactory.chooseClassMapBuilderFactory(aType, bType);
   ...
   */
  public <A, B> BoundMapperFacade<A, B> getMapperFacade(
  ...
  /**
   * MapperFactoryBuilder provides an extensible Builder definition usable for providing your own
   * Builder class for subclasses of DefaultMapperFactory.<br>
   * <br>
   ...
   * @param <B>
   */
  public abstract static class MapperFactoryBuilder<
      F extends DefaultMapperFactory, B extends MapperFactoryBuilder<F, B>> {
    ...
    /**
     * The configured value of whether or not to map null values; if false, they will be ignored,
     * and any existing value is unchanged in case of null.
     */
    protected Boolean mapNulls;
    /**
     * The configured value of whether the full state of the core Orika mapping objects should be
     * printed on exception
     */
    protected Boolean dumpStateOnException;
    /**
     * The configured default value for the 'favorExtension' option on registered class-maps (when
     * one has not been explicitly specified).
     */
    protected Boolean favorExtension;
    /**
     * The configured value for whether full field context should be captured upon mapping of every
     * field.
     */
    protected Boolean captureFieldContext;
    /**
     * Internal Property to always use {@link MultipleMapperWrapper} even if there are no custom
     * ObjectFactory and only one Mapper found.
     *
     * <p>This makes no sense for production: Because of the (small) performance penalty the {@link
     * MultipleMapperWrapper} should only be use when necessary.
     *
     * <p>But for Testing the MultipleMapperWrapper should be use in one TestSuite for all TestCase
     * to test as much scenarios as possible.
     */
    protected boolean alwaysCreateMultipleMapperWrapper;
    ...
    public MapperFactoryBuilder() {
      ...
      alwaysCreateMultipleMapperWrapper =
          valueOf(getProperty("ma.glasnost.orika.alwaysCreateMultipleMapperWrapper", "false"));
      codeGenerationStrategy = new DefaultCodeGenerationStrategy();
    }

    /** @return an appropriately type-cast reference to <code>this</code> MapperFactoryBuilder */
    ...
    /**
     * Configure whether to use built-in converters with the generated MapperFactory<br>
     *
    ...
    /**
     * Configure whether to dump the current state of the mapping infrastructure objects upon
    ...
    /**
     * Configure whether to favorExtension by default in registered class-maps (when a value has not
     * been explicitly specified on the class-map builder).
     *
     ...
     *
     * <p>If <code>true</code>, the result is that the following calls will return a meaningful
     * value, relative to the currently mapped source and destination fields:
     *
     * <ul>
     ...
     * <p>
     *
     * <p>If <code>false</code>, these methods will return <code>null</code>.
     *
    ...
    /**
     * Get a reference to the CodeGenerationStrategy associated with this MapperFactory, which may
     * be used to configure/customize the individual mapping Specifications that are used to
     * generate code for the various mapping scenarios.
     *
    ...
    /**
     * Configure a CodeGenerationStrategy with this MapperFactory, which may be used to
     * configure/customize the individual mapping Specifications that are used to generate code for
    ...
    /**
     * @return a new instance of the Factory for which this builder is defined. The construction
     *     should be performed via the single-argument constructor which takes in a builder; no
     *     initialization code should be performed here, as it will not be inherited by subclasses;
     *     instead, place such initialization (defaults, etc.) in the Builder's constructor.
     */
  ...
  /**
   * ConverterFactoryFacade is a nested intercepter class for ConverterFactory that listens for
   * registry of new converters and calls the appropriate change event on MapperFacade if the
