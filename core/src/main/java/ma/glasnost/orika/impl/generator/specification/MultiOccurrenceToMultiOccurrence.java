 ...
 * fields to one or more multi-occurrence destination fields.
 */
public class MultiOccurrenceToMultiOccurrence implements AggregateSpecification {
   ...
   * multi-occurrence fields in the source type to one or more multi-occurrence fields in the
   * destination type.
   *
  ...
  public boolean appliesTo(FieldMap fieldMap) {
    return fieldMap.getSource().getContainer() != null
   ...
   */
  private String generateMultiOccurrenceMapping(
      NodeList sourceNodes, NodeList destNodes, Set<FieldMap> subFields, SourceCodeContext code) {
    ...
    for (Node destRef : destNodes) {
      ...
      if (!destRef.isLeaf()) {
        ...
        if (!"".equals(sourcesNotNull.toString())) {
           ...
           */
          out.append(statement(destRef.newDestination.assign("null")));
        }
    ...
    while (!stack.isEmpty()) {
      ...
      if (!currentNode.isLeaf() && srcNode != null) {
         ...
         */
        String currentElementNull =
            currentNode.elementRef.isPrimitive()
                ? currentNode.nullCheckFlag.toString()
                : currentNode.elementRef.isNull();
        String currentElementComparator =
            code.currentElementComparator(srcNode, currentNode, sourceNodes, destNodes);
        String or =
            (!"".equals(currentElementNull) && !"".equals(currentElementComparator)) ? " || " : "";

        if (mapperFactory
                .getConverterFactory()
                .canConvert(srcNode.elementRef.type(), currentNode.elementRef.type())
            || currentNode.elementRef.type().isImmutable()) {

          append(
              out,
              (currentNode.elementRef.isPrimitive()
                  ? currentNode.nullCheckFlag.assign("false")
                  : ""),
          ...
          append(
              ...
              currentNode.elementRef.assign(
                  code.newObject(srcNode.elementRef, currentNode.elementRef.type())),
              currentNode.shouldAddToCollectorFlag.assign("true"),
              "}");
        }
      }
      ...
      if (srcNode != null) {
        if (currentNode.value != null) {
          ...
          if (srcNode.parent != null
            ...
            String assignNull =
                (currentNode.parent.elementRef.isPrimitive()
                    ? currentNode.parent.nullCheckFlag.assign("true")
                    : currentNode.parent.elementRef.assign("null"));
            if (mapperFactory
                .getConverterFactory()
                .canConvert(
                    srcNode.parent.elementRef.type(), currentNode.parent.elementRef.type())) {
              append(
                  out,
                  (currentNode.parent.isRoot()
                      ? currentNode.parent.newDestination.add(currentNode.parent.elementRef)
                      : currentNode.parent.multiOccurrenceVar.add(currentNode.parent.elementRef)),
                  assignNull);
            } else {
              append(
                  out,
      ...
      if (destRef.isRoot() && !destRef.isLeaf()) {
        if (destRef.multiOccurrenceVar.isArray()) {
          ...
          append(
              ...
              format(
                  "if (%s && %s) {",
                  destRef.newDestination.notNull(), destRef.newDestination.notEmpty()),
          ...
          append(
              ...
              destRef.multiOccurrenceVar.assignIfPossible(
                  destRef.multiOccurrenceVar.newInstance(sizeExpr)),
              "} else {\n",
  ...
  private boolean mapFields(
      Node currentNode, Node srcNode, StringBuilder out, SourceCodeContext code) {
    ...
    if (elRef != null
        && !elRef.isPrimitive()
        && !elRef.type().isImmutable()
        && srcNode.parent != null) {
      // assure instance exists for the element reference
      append(
          out,
          format("if((%s)) { \n", elRef.isNull()),
          elRef.assign(code.newObject(srcNode.parent.elementRef, elRef.type())),
          "}");
    }
    out.append(mapStmt);
    ...
    Type<?> parentElementType =
        currentNode.parent != null
            ? currentNode.parent.elementRef.type()
            : TypeFactory.TYPE_OF_OBJECT;

    return d.type().equals(parentElementType)
        && mapperFactory.getConverterFactory().canConvert(s.type(), d.type());
  }
  ...
  /**
   * Register the ClassMaps needed to map this pair of source and destination nodes.
   *
   * @param sourceNodes
   * @param destNodes
   */
  private void registerClassMaps(NodeList sourceNodes, NodeList destNodes) {
     ...
     * mapping
     */
    Map<MapperKey, ClassMapBuilder<?, ?>> builders =
        new HashMap<MapperKey, ClassMapBuilder<?, ?>>();
    ...
    while (!stack.isEmpty()) {
      ...
      if (srcNode != null
        ...
        MapperKey key =
            new MapperKey(srcNode.parent.elementRef.type(), currentNode.parent.elementRef.type());
