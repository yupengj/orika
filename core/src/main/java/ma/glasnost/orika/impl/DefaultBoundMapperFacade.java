...
class DefaultBoundMapperFacade<A, B> implements BoundMapperFacade<A, B> {
   ...
   * @param typeOfB
   */
  DefaultBoundMapperFacade(
      MapperFactory mapperFactory,
      MappingContextFactory contextFactory,
      java.lang.reflect.Type typeOfA,
      java.lang.reflect.Type typeOfB) {
  ...
  /**
   * BoundStrategyCache attempts to optimize caching of MappingStrategies for a particular situation
   * based on the assumption that the most common case involves mapping with a single source type
   * class (no polymorphism within most BoundMapperFacades); it accomplishes this by caching a
   * single MappingStrategy as a default case which is always fast at hand, falling back to a
   * (small) hashmap of backup strategies, keyed by source Class (since all of the other inputs to
   * resolve the strategy are fixed to the BoundStrategyCache instance).
   */
  private static class BoundStrategyCache {
    protected final ConcurrentHashMap<Class<?>, MappingStrategy> strategies =
        new ConcurrentHashMap<Class<?>, MappingStrategy>(2);
      ...
      if (defaultStrategy != null && sourceClass.equals(idClass)) {
        ...
        synchronized (this) {
          if (defaultStrategy == null) {
            defaultStrategy =
                mapperFacade.resolveMappingStrategy(sourceObject, aType, bType, inPlace, context);
            idClass = sourceClass;
