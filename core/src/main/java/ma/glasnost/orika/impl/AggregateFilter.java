...
/**
 * AggregateFilter provides the capability to combine multiple filters together to act as a single
 * filter
 *
 * @author mattdeboer
 */
public class AggregateFilter implements Filter<Object, Object> {
  ...
  public <S, D> boolean shouldMap(
    ...
    for (Filter<Object, Object> filter : filters) {
      if (!filter.shouldMap(
          sourceType, sourceName, source, destType, destName, dest, mappingContext)) {
        shouldMap = false;
        break;
      }
    }
      ...
      if (filter.filtersDestination()) {
        value =
            filter.filterDestination(
                value, sourceType, sourceName, destType, destName, mappingContext);
      }
    }
    return value;
  }

  public <S> S filterSource(
      S sourceValue,
      Type<S> sourceType,
      String sourceName,
      Type<?> destType,
    ...
    for (Filter<Object, Object> filter : filters) {
      if (filter.filtersDestination()) {
        value =
            filter.filterSource(value, sourceType, sourceName, destType, destName, mappingContext);
      }
    }
    return value;
  }
