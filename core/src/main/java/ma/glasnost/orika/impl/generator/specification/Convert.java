 ...
 * applied when the converter is a CopyByReferenceConverter -- we applied the code to assign the
 * reference directly rather than making an extra method call.
 */
public class Convert extends AbstractSpecification {
  ...
  public String generateEqualityTestCode(
    ...
    if (source.getConverter() instanceof CopyByReferenceConverter) {
      ...
      if (destination.type().isPrimitive() && source.type().isPrimitive()) {
        ...
        return format(
            "(%s != null && %s == %s.%sValue())",
            source, destination, source, source.type().getName());
      } else if (source.type().isPrimitive()) {
        return format(
            "(%s != null && %s.%sValue() == %s)",
            destination, destination, destination.type().getPrimitiveType().getName(), source);
      } else {
      ...
      if (destination.type().isPrimitive()) {
        ...
        return format(
            "(%s == ((%s)%s.convert(%s, %s, mappingContext)).%sValue())",
            destination,
            wrapperType,
        ...
        return format(
            "(%s == %s.convert(%s, %s, mappingContext))",
            destination.asWrapper(),
            code.usedConverter(source.getConverter()),
            source.asWrapper(),
            code.usedType(destination));
      } else {
        return format(
            "(%s != null && %s.equals(%s.convert(%s, %s, mappingContext)))",
            destination,
            destination,
            code.usedConverter(source.getConverter()),
            source.asWrapper(),
  ...
  public String generateMappingCode(
    ...
    if (source.getConverter() instanceof CopyByReferenceConverter) {
      ...
      statement =
          destination.assignIfPossible(
              "%s.convert(%s, %s, mappingContext)",
              code.usedConverter(source.getConverter()),
              source.asWrapper(),
              code.usedType(destination));
    ...
    if (!source.isNullPossible()
        || (canHandleNulls && shouldSetNull && "".equals(destinationNotNull))) {
      return statement(statement);
    } else {
      String elseSetNull =
          shouldSetNull
              ? (" else " + destinationNotNull + "{ \n" + destination.assignIfPossible("null"))
                  + ";\n }"
              : "";
