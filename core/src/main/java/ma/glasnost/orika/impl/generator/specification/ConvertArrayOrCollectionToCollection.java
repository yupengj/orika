...
/**
 * ConvertArrayOrCollectionToCollection handles mapping of an Array or Collection to a Collection
 * using a Converter which has been specified at the element level.
 */
public class ConvertArrayOrCollectionToCollection extends AbstractSpecification {
  ...
  public String generateMappingCode(
    ...
    String assureInstanceExists =
        format(
            "if((%s)) { \n %s; \n}",
            destination.isNull(), destination.assign(code.newObject(source, destination.type())));
    ...
    append(
        ...
        format(
            "%s.add(%s.convert((%s)%s, %s, mappingContext))",
            moDest,
    ...
    String mapNull =
        shouldMapNulls(fieldMap, code)
            ...
            : "";
    return format(
        " %s { %s; %s; } %s", source.ifNotNull(), assureInstanceExists, convertCollection, mapNull);
  }
}
