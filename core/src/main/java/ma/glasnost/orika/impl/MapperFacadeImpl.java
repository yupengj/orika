...
public class MapperFacadeImpl implements MapperFacade {
  ...
  public MapperFacadeImpl(
      ...
      final MappingContextFactory contextFactory,
  ...
  /**
   * Normalize the source type based on the registered converters, mappers and accessible super
   * types, as well as available unenhancers
   *
   * @param sourceObject
   * @param sourceType
  ...
  private <S, D> Type<S> normalizeSourceType(
      final S sourceObject, Type<S> sourceType, final Type<D> destinationType) {
    ...
    if (sourceType != null) {
      if (destinationType != null
        ...
        if (sourceType.isConcrete()) {
          ...
          resolvedType =
              unenhanceStrategy.unenhanceType(
                  sourceObject, resolveTypeOf(sourceObject, sourceType));
        }
      }
    } else {
  ...
  public <S, D> D map(
      final S sourceObject, final Type<S> sourceType, final Type<D> destinationClass) {
  ...
  public <S, D> MappingStrategy resolveMappingStrategy(
      ...
      final java.lang.reflect.Type initialSourceType,
      final java.lang.reflect.Type initialDestinationType,
      final boolean mapInPlace,
      final MappingContext context) {

    Key key =
        new Key(getClass(sourceObject), initialSourceType, initialDestinationType, mapInPlace);
    ...
    if (strategy == null) {
      ...
      Type<S> sourceType =
          (Type<S>)
              (initialSourceType != null
                  ? TypeFactory.valueOf(initialSourceType)
                  : typeOf(sourceObject));
      Type<D> destinationType = TypeFactory.valueOf(initialDestinationType);

      MappingStrategyRecorder strategyRecorder =
          new MappingStrategyRecorder(key, unenhanceStrategy);

      final Type<S> resolvedSourceType =
          normalizeSourceType(sourceObject, sourceType, destinationType);
      ...
      if (!mapInPlace && canCopyByReference(destinationType, resolvedSourceType)) {
        ...
        strategyRecorder.setResolvedConverter(
            mapperFactory.getConverterFactory().getConverter(resolvedSourceType, destinationType));

      } else {
        strategyRecorder.setInstantiate(true);
        Type<? extends D> resolvedDestinationType =
            resolveDestinationType(context, sourceType, destinationType, resolvedSourceType);

        strategyRecorder.setResolvedDestinationType(resolvedDestinationType);
        strategyRecorder.setResolvedMapper(
            resolveMapper(resolvedSourceType, resolvedDestinationType, context));
        if (!mapInPlace) {
          strategyRecorder.setResolvedObjectFactory(
              mapperFactory.lookupObjectFactory(
                  resolvedDestinationType, resolvedSourceType, context));
        }
      }
  ...
  private <S, D> Type<? extends D> resolveDestinationType(
      MappingContext context,
      Type<S> sourceType,
      Type<D> destinationType,
      Type<S> resolvedSourceType) {
    Type<? extends D> resolvedDestinationType =
        mapperFactory.lookupConcreteDestinationType(resolvedSourceType, destinationType, context);
  ...
  public <S, D> D map(
    ...
    try {
      ...
      if (existingResult == null) {
        if (strategy == null) {
          strategy =
              resolveMappingStrategy(sourceObject, sourceType, destinationType, false, context);
        }
  ...
  /**
   * Resolves whether the given mapping operation can use copy-by-reference semantics; should be
   ...
   * @return
   */
  private <D, S> boolean canCopyByReference(
      final Type<D> destinationType, final Type<S> resolvedSourceType) {
    if (resolvedSourceType.isImmutable()
        && (destinationType.isAssignableFrom(resolvedSourceType))) {
      return true;
    } else if (resolvedSourceType.isPrimitiveWrapper()
        && resolvedSourceType
            .getRawType()
            .equals(ClassUtil.getWrapperType(destinationType.getRawType()))) {
      return true;
    } else if (resolvedSourceType.isPrimitive()
        && destinationType
            .getRawType()
            .equals(ClassUtil.getWrapperType(resolvedSourceType.getRawType()))) {
      return true;
    } else {
      return false;
    }
  ...
  public <S, D> void map(
      final S sourceObject, final D destinationObject, final MappingContext context) {
  ...
  private <S, D> void map(
    ...
    try {
      if (strategy == null) {
        strategy =
            resolveMappingStrategy(sourceObject, null, destinationObject.getClass(), true, context);
      }
      strategy.map(sourceObject, destinationObject, context);
  ...
  public final <S, D> Set<D> mapAsSet(
      final Iterable<S> source, final Type<S> sourceType, final Type<D> destinationType) {
    MappingContext context = contextFactory.getContext();
    try {
  ...
  public final <S, D> List<D> mapAsList(
      final Iterable<S> source, final Type<S> sourceType, final Type<D> destinationType) {
    MappingContext context = contextFactory.getContext();
    try {
      return (List<D>)
          mapAsCollection(source, sourceType, destinationType, new ArrayList<D>(), context);
    } finally {
      contextFactory.release(context);
    }
  }
  ...
  public <S, D> D[] mapAsArray(
    ...
    int i = 0;
    ElementStrategyContext<S, D> elementContext =
        new ElementStrategyContext<S, D>(context, sourceType, destinationType);
  ...
  public <S, D> List<D> mapAsList(
      final S[] source, final Type<S> sourceType, final Type<D> destinationType) {
    MappingContext context = contextFactory.getContext();
    try {
  ...
  public <S, D> void mapAsCollection(
      final Iterable<S> source,
      final Collection<D> destination,
      final Type<S> sourceType,
      final Type<D> destinationType,
      final MappingContext context) {
    if (source == null) {
  ...
  private Mapper<Object, Object> resolveMapper(
      final Type<?> sourceType, final Type<?> destinationType, final MappingContext context) {
    ...
    if (mapper == null) {
      throw new IllegalStateException(
          String.format(
              "Cannot create a mapper for classes : %s, %s", destinationType, sourceType));
    }

    if ((!mapper.getAType().equals(sourceType) && mapper.getAType().equals(destinationType))
        || (!mapper.getAType().isAssignableFrom(sourceType)
            && mapper.getAType().isAssignableFrom(destinationType))) {
      mapper = ReversedMapper.reverse(mapper);
    }
    return mapper;
  }

  private <S, D> D newObject(
      final S sourceObject,
      final Type<? extends D> destinationType,
      final MappingContext context,
      final MappingStrategyRecorder strategyBuilder) {

    final ObjectFactory<? extends D> objectFactory =
        mapperFactory.lookupObjectFactory(
            destinationType, TypeFactory.valueOf(sourceObject.getClass()), context);
   ...
   */
  public <S, D> D newObject(
      final S sourceObject, final Type<? extends D> destinationType, final MappingContext context) {
   ...
   * @return
   */
  private <S, D> Collection<D> mapAsCollection(
      final Iterable<S> source,
      final Type<S> sourceType,
      final Type<D> destinationType,
      final Collection<D> destination,
  ...
  @SuppressWarnings("unchecked")
  public <S, D> D map(
      final S sourceObject, final Class<D> destinationClass, final MappingContext context) {
  ...
  public <S, D> Set<D> mapAsSet(
      final Iterable<S> source, final Class<D> destinationClass, final MappingContext context) {
  ...
  public <S, D> Set<D> mapAsSet(
      final S[] source, final Class<D> destinationClass, final MappingContext context) {
    return mapAsSet(
        source, componentTypeOf(source), TypeFactory.valueOf(destinationClass), context);
  }
  ...
  public <S, D> D[] mapAsArray(
      final D[] destination, final Iterable<S> source, final Class<D> destinationClass) {
    return mapAsArray(
        destination, source, elementTypeOf(source), TypeFactory.valueOf(destinationClass));
  }

  public <S, D> D[] mapAsArray(
      final D[] destination, final S[] source, final Class<D> destinationClass) {
    return mapAsArray(
        destination, source, componentTypeOf(source), TypeFactory.valueOf(destinationClass));
  }
  ...
  public <S, D> D[] mapAsArray(
      ...
      final MappingContext context) {
    return mapAsArray(
        destination, source, elementTypeOf(source), TypeFactory.valueOf(destinationClass), context);
  }
  ...
  public <S, D> D convert(
      ...
      MappingContext context) {
    return convert(
        source, typeOf(source), TypeFactory.valueOf(destinationClass), converterId, context);
  }

  public <Sk, Sv, Dk, Dv> Map<Dk, Dv> mapAsMap(
      final Map<Sk, Sv> source,
      final Type<? extends Map<Sk, Sv>> sourceType,
  ...
  /**
   * Provides mapping specific to elements of a multi-occurrence object, such as an array,
   * collection, or map. Hadles caching of the resolved strategy based on the class type of the
   * source object using the ElementStrategyContext.
   *
   * <p>Additionally, ensures that the MappingContext instance associated has source, destination,
   * and strategy consistently set.
   *
   * @param source
   * @param context
   * @return
    ...
    if (context.strategy == null || !sourceClass.equals(context.sourceClass)) {
      context.strategy =
          resolveMappingStrategy(
              source, context.sourceType, context.destinationType, false, context.mappingContext);
  ...
  public <Sk, Sv, Dk, Dv> Map<Dk, Dv> mapAsMap(
     ...
     * possible
     */
    ElementStrategyContext<Sk, Dk> keyContext =
        new ElementStrategyContext<Sk, Dk>(
            context, sourceType.<Sk>getNestedType(0), destinationType.<Dk>getNestedType(0));
    ElementStrategyContext<Sv, Dv> valContext =
        new ElementStrategyContext<Sv, Dv>(
            context, sourceType.<Sv>getNestedType(1), destinationType.<Dv>getNestedType(1));
  ...
  public <S, Dk, Dv> Map<Dk, Dv> mapAsMap(
    ...
    Type<?> entryType =
        TypeFactory.valueOf(
            Entry.class, destinationType.getNestedType(0), destinationType.getNestedType(1));
    ElementStrategyContext<S, Entry<Dk, Dv>> elementContext =
        new ElementStrategyContext<S, Entry<Dk, Dv>>(
            context, sourceType, (Type<Entry<Dk, Dv>>) entryType);
  ...
  public <S, Dk, Dv> Map<Dk, Dv> mapAsMap(
    ...
    ElementStrategyContext<S, MapEntry<Dk, Dv>> elementContext =
        new ElementStrategyContext<S, MapEntry<Dk, Dv>>(context, sourceType, entryType);
  ...
  public <Sk, Sv, D> List<D> mapAsList(
    ...
    return (List<D>)
        mapAsCollection(
            MapEntry.entrySet(source), entryType, destinationType, destination, context);
  }
  ...
  public <Sk, Sv, D> Set<D> mapAsSet(
    ...
    return (Set<D>)
        mapAsCollection(source.entrySet(), entryType, destinationType, destination, context);
  }
  ...
  public <S, D> void mapAsCollection(
      final Iterable<S> source, final Collection<D> destination, final Class<D> destinationClass) {
    MappingContext context = contextFactory.getContext();
    try {
  ...
  public <S, D> void mapAsCollection(
      final S[] source, final Collection<D> destination, final Class<D> destinationClass) {
    MappingContext context = contextFactory.getContext();
    try {
      mapAsCollection(source, destination, destinationClass, context);
    } finally {
  ...
  public <S, D> void mapAsCollection(
    ...
    mapAsCollection(
        source,
        destination,
        (Type<S>) TypeFactory.valueOf(source.getClass().getComponentType()),
        TypeFactory.valueOf(destinationClass),
        context);
  }
  ...
  private <T> Type<T> componentTypeOf(final T[] object) {
    return object == null
        ? null
        : TypeFactory.valueOf((Class<T>) object.getClass().getComponentType());
  }

  /**
   * Resolve the Type for the given object, using the provided referenceType to resolve the actual
   * type arguments.
   *
   * @param object
   * @param referenceType
  ...
  private <T> Type<T> resolveTypeOf(final T object, Type<?> referenceType) {
    return object == null
        ? null
        : TypeFactory.resolveValueOf((Class<T>) object.getClass(), referenceType);
  }
  ...
  /**
   * A context object used to track the strategy specific to mapping the elements of a
   * multi-occurrence object.
   *
   * @param <S>
   * @param <D>
   */
  private static class ElementStrategyContext<S, D> {
    ...
    public ElementStrategyContext(
        MappingContext mappingContext, Type<S> sourceType, Type<D> destinationType) {
      super();
