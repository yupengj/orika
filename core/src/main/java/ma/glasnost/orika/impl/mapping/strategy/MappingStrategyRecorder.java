...
/**
 * MappingStrategyRecorder is used to record the important details regarding the branch logic and
 * component lookups performed for a given set of input types.<br>
 * After recording these details, it can be used to generate an appropriate MappingStrategy instance
 * which can be cached and reused for that particular set of inputs.
 */
public class MappingStrategyRecorder {
  ...
  /**
   * @return a new instance of the MappingStrategy which can "playback" the route taken to map a
   *     given set of inputs.
   */
  public synchronized MappingStrategy playback() {
    ...
    if (copyByReference) {
      ...
      resolvedStrategy =
          new UseConverterStrategy(
              resolvedSourceType, resolvedDestinationType, resolvedConverter, unenhanceStrategy);
    } else {
      ...
      if (resolvedObjectFactory != null) {
        resolvedStrategy =
            new InstantiateAndUseCustomMapperStrategy(
                resolvedSourceType,
        ...
        resolvedStrategy =
            new MapExistingAndUseCustomMapperStrategy(
                resolvedSourceType, resolvedDestinationType, resolvedMapper, unenhanceStrategy);
      }
    }
