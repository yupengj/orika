...
public class ArrayOrCollectionToMap extends AbstractSpecification {
  ...
  public String generateMappingCode(
    ...
    MultiOccurrenceVariableRef newDest =
        new MultiOccurrenceVariableRef(d.type(), "new_" + d.name());

    append(out, s.ifNotNull() + " {");
    ...
    VariableRef element =
        new VariableRef(s.elementType(), "source" + StringUtil.capitalize(s.name()) + "Element");

    @SuppressWarnings("unchecked")
    Type<MapEntry<Object, Object>> entryType =
        MapEntry.concreteEntryType((Type<? extends Map<Object, Object>>) d.type());

    VariableRef newEntry =
        new VariableRef(entryType, "source" + StringUtil.capitalize(s.name()) + "Entry");
    ...
    if (s.isArray()) {
      if (code.isDebugEnabled()) {
        code.debugField(
            ...
            "mapping "
                + s.elementTypeName()
                + "[] to Map<"
                + d.type().getNestedType(0)
                + ", "
                + d.type().getNestedType(1)
                + ">");
      }
      append(
          out,
          format(
              "for( int entryIndex = 0, entryLen = %s.length; entryIndex < entryLen; ++entryIndex ) {\n",
              s),
          element.declare("%s[entryIndex]", s),
          newEntry.declare(
              "mapperFacade.map(%s, %s, %s, mappingContext)",
              element, code.usedType(element), code.usedType(newEntry)),
          "\n",
      ...
      append(
          out,
          format(
              "for( java.util.Iterator entryIter = %s.iterator(); entryIter.hasNext(); ) {\n", s),
