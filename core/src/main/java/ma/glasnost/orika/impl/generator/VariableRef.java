 ...
 * methods to properly set it's value and interrogate it's underlying property or type. It also
 * returns a properly type-safe cast of it as the toString() method, so it can safely be used
 * directly as a replacement parameter for source code statements.
 */
public class VariableRef {

  private static final Set<Class<?>> OPTIONAL_CLASSES =
      ...
      if (type.isPrimitive()) {
        if (type.getRawType() == Character.TYPE) {
          ...
          castValue =
              format(
                  "%s.valueOf(\"\"+%s).%sValue()",
                  type.getWrapperType().getCanonicalName(), castValue, type);
        }
    ...
    if (type.isPrimitive()) {
      if (value.isWrapper()) {
        ...
        castValue =
            format(
                "%s.valueOf(\"\"+%s).%sValue()",
                type.getWrapperType().getCanonicalName(), castValue, typeName);
      }
  ...
  public static String getDefaultValue(Class<?> clazz) {
    ...
    else if (Character.TYPE.equals(clazz)) return "'\\u0000'";
  ...
  private static String isNull(Property property, String name) {
    if (property == null) {
      ...
      if (property.isListElement()) {
        return "("
            + unwrap(getGetter(property, name))
            + ".size() <= "
   ...
   * @param variableExpression the String value to use for the variable on which the getter is
   *     called
   * @return
   */
  protected static String getSetter(final Property property, final String variableExpression) {
    ...
    return var
        + (property.isArrayElement()
                || "".equals(property.getName())
                || property.getSetter().startsWith("[")
            ? ""
            : ".")
        + property.getSetter();
  }
  ...
  public Type<?> elementType() {
    ...
    if (type.getRawType().isArray()) {
      ...
      return property != null
          ...
          : ((Type<?>) type.getActualTypeArguments()[0]);
    }
  }
  ...
  public String assign(String value, Object... replacements) {
    if (setter() != null) {
      ...
      throw new IllegalArgumentException(
          "Attempt was made to generate assignment/setter code for ["
              + name
              + "."
              + (property != null ? property : type)
              + "] which has no setter/assignment method");
    }
  }
    ...
    if (setter() != null) {
      ...
      if (value.type().isPrimitive() && type.isPrimitiveWrapper()) {
        ...
        expr =
            format(
                "((%s)%s).%sValue()",
                ClassUtil.getWrapperType(rawType()).getCanonicalName(),
                expr,
        ...
        if (type.getComponentType().isPrimitive() && value.elementType().isPrimitiveWrapper()) {
          String wrapperType =
              ClassUtil.getWrapperType(rawType().getComponentType()).getCanonicalName();
          expr = format("(%s[])%s.toArray(new %s[0])", wrapperType, expr, wrapperType);
        } else {
          expr =
              format(
                  "(%s[])%s.toArray(new %s[0])",
                  value.elementType().getCanonicalName(),
                  expr,
                  value.elementType().getCanonicalName());
        }
   ...
   * Returns Java code which declares this variable, initialized with the provided value.
   *
   * @param initialValueRef the VariableRef instance which describes the initial value of the
   *     receiver
   * @return the code which declares this variable, and explicitly assigns the provided value.
   */
    ...
    if (property != null && property.hasPath()) {
      ...
      if (propPath.length > 1) {
        ...
        for (int i = 1; i < propPath.length; ++i) {
          ...
          path.add(
              new VariableRef(
                  new NestedProperty(join(propPath, ".", 0, i + 1), propPath[i], nestedPath),
                  name));
        }
      } else {
  ...
  /**
   * @return true if it is possible for this variable to be null at the current state within code
   */
  ...
  /**
   * Return Java code which avoids a NullPointerException when accessing this variable reference; if
   * it is not backed by a nested property, this method returns the empty string.
   *
   * @return
   */
  ...
  /**
   * Return Java code which checks if this path is null; if it is not backed by a nested property,
   ...
   *     property's expression is valid for this VariableRef's type
   */
