...
public class ArrayOrCollectionToCollection extends AbstractSpecification {
  ...
  public String generateMappingCode(
    ...
    if (destinationElementClass == null) {
      final String dc =
          destination.getOwner() == null ? "null" : destination.getOwner().rawType().getName();
      throw new MappingException(
          "cannot determine runtime type of destination collection " + dc + "." + d.name());
    }
    ...
    MultiOccurrenceVariableRef newDest =
        new MultiOccurrenceVariableRef(d.type(), "new_" + d.validVariableName());
    if (d.isAssignable()) {
    ...
    if (s.isArray()) {
      if (code.isDebugEnabled()) {
        code.debugField(
            fieldMap,
            "mapping " + s.elementTypeName() + "[] to Collection<" + d.elementTypeName() + ">");
      }
      ...
      if (s.elementType().isPrimitive()) {
        ...
        out.append(
            statement(
                "%s.addAll(mapperFacade.mapAsList(asList(%s), %s.class, mappingContext));",
                newDest, s, d.elementTypeName()));
      }
    } else {
      if (code.isDebugEnabled()) {
        code.debugField(
      ...
      append(
          ...
          "\n",
          format(
              "%s.addAll(mapperFacade.mapAs%s(%s, %s, %s, mappingContext))",
              newDest,
              d.collectionType(),
              s,
              code.usedType(s.elementType()),
              code.usedType(d.elementType())));
    }
    if (fieldMap.getInverse() != null) {
      final MultiOccurrenceVariableRef inverse =
          new MultiOccurrenceVariableRef(fieldMap.getInverse(), "orikaCollectionItem");
      ...
      if (fieldMap.getInverse().isCollection()) {
        append(
            ...
            format(
                "    %s { %s; }",
                inverse.ifNull(), inverse.assignIfPossible(inverse.newCollection())),
