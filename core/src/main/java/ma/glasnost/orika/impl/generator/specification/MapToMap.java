...
public class MapToMap extends AbstractSpecification {
  ...
  public String generateMappingCode(
    ...
    if (code.isDebugEnabled()) {
      code.debugField(
          ...
          "mapping from Map<"
              ...
              + "> to Map<"
              + destination.type().getNestedType(0)
              + ", "
    ...
    MultiOccurrenceVariableRef newDest =
        new MultiOccurrenceVariableRef(destination.type(), "new_" + destination.name());
    if (d.isAssignable()) {
    ...
    VariableRef newKey =
        new VariableRef(d.mapKeyType(), "new" + StringUtil.capitalize(d.name()) + "Key");
    VariableRef newVal =
        new VariableRef(d.mapValueType(), "new" + StringUtil.capitalize(d.name()) + "Val");
    VariableRef entry =
        new VariableRef(
            TypeFactory.valueOf(Map.Entry.class),
            "source" + StringUtil.capitalize(d.name()) + "Entry");
    ...
    append(
        ...
        code.mapFields(
            FieldMapBuilder.mapValues(s.mapValueType(), d.mapValueType()), sourceVal, newVal),
