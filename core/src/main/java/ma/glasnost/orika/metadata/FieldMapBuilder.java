...
/**
 * FieldMapBuilder is used in cooperation with the ClassMapBuilder fluent api to configure the
 * details of a given FieldMap instance. <br>
 * <br>
 * A FieldMapBuilder instance is obtained from a ClassMapBuilder instance via the <code>
 * fieldMap(...)</code> method; it may then be used to further configure that field mapping.
 * Finally, the <code>add()</code> is used to add the associated field mapping to the containing
 * ClassMapBuilder.
 *
 * @param <A>
 * @param <B>
...
public class FieldMapBuilder<A, B> {
  ...
  FieldMapBuilder(
      final ClassMapBuilder<A, B> classMapBuilder,
      final String a,
      final String b,
      final boolean byDefault,
      final Boolean sourceMappedOnNull,
  ...
  FieldMapBuilder(
    ...
    this(
        classMapBuilder,
        classMapBuilder.resolveProperty(aType, a),
        classMapBuilder.resolveProperty(bType, b),
        byDefault,
        sourceMappedOnNull,
        destinationMappedOnNull);
  }
  ...
  public static FieldMap mapKeys(final Type<?> aType, final Type<?> bType) {
    ...
    return new FieldMap(
        aProperty, bProperty, null, null, MappingDirection.A_TO_B, false, null, false, null, null);
  }
  ...
  /**
   * Adds the FieldMap configured by this builder to it's containing ClassMapBuilder; use this
   * method to complete specifications on a given field mapping and return to the containing
   * builder.
   *
  ...
  public FieldMapBuilder<A, B> aInverse(final String aInverse) {
    final Type<?> type =
        aProperty.isCollection() ? aProperty.getElementType() : aProperty.getType();
  ...
  public FieldMapBuilder<A, B> bInverse(final String bInverse) {
    final Type<?> type =
        bProperty.isCollection() ? bProperty.getElementType() : bProperty.getType();
  ...
  /**
   * @param sourceMappedOnNull true|false to indicate whether the source property of this field map
   *     should be set to null (when mapping in the reverse direction) if the destination property's
   *     value is null
   * @return this FieldMapBuilder
   */
  ...
  /**
   * @param destinationMappedOnNull true|false to indicate whether the destination property of this
   *     field map should be set to null (when mapping in the forward direction) if the source
  ...
  /**
   * Specify that the configured field mapping (property) should only be used when mapping in the
  ...
  /**
   * Specify that the converter (which was previously registered with the specified id) should be
   * applied to this specific field mapping.
   *
   ...
   * <i>When dealing with legacy code, prior to Java 1.5 you can add element type of collections,
   * this is not required when using generics</i>
   *
