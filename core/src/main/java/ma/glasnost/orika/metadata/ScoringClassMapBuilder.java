...
/**
 * ScoringClassMapBuilder is an extension of the basic ClassMapBuilder that attempts to compute a
 * best-fit matching of all properties (at every level of nesting) of one type to another, based on
 * various metrics used to measure a given property match.<br>
 * <br>
 * Since this builder generates mappings based on scoring matches, it cannot always guess the
 * correct mappings; be sure to test and double-check the mappings generated to assure they match
 * expectations.<br>
 * <br>
 * Note: levenshtein distance implementation is pulled from code found in Apache Commons Lang
 * <em>org.apache.commons.lang.StringUtils</em>, which is based on the implementation provided by
 * Chas Emerick <a
...
public class ScoringClassMapBuilder<A, B> extends ClassMapBuilder<A, B> {
   ...
   * @param defaults
   */
  protected ScoringClassMapBuilder(
      Type<A> aType,
      Type<B> bType,
      MapperFactory mapperFactory,
      PropertyResolverStrategy propertyResolver,
      DefaultFieldMapper[] defaults,
      PropertyMatchingWeights matchingWeights) {
  ...
  public ClassMapBuilder<A, B> byDefault(
      MappingDirection direction, DefaultFieldMapper... withDefaults) {
        ...
        for (final Entry<String, Property> propertyB : propertiesForB.entrySet()) {
          if (!propertyB.getValue().getName().equals("class")) {
            FieldMatchScore matchScore =
                new FieldMatchScore(propertyA.getValue(), propertyB.getValue(), matchingWeights);
    ...
    for (FieldMatchScore score : matchScores) {
      ...
      if (!this.getMappedPropertiesForTypeA().contains(score.propertyA.getExpression())
        ...
        if (score.meetsMinimumScore()) {
          fieldMap(score.propertyA.getExpression(), score.propertyB.getExpression())
              .direction(direction)
              .add();
  ...
  /**
   * PropertyMatchingWeights is a class used to describe how different matching scenarios should be
   * weighted when computing a match score for a set of properties.
   */
  public static final class PropertyMatchingWeights {
    ...
    /**
     * @return the weight associated with the number of words found in common between two property
     *     expressions
     */
    ...
    /**
     * Set the weight associated with the number of words found in common between two property
     * expressions
     *
     * @param weight the weight associated with the number of words found in common
     * @return this instance of PropertyMatchingWeights between two property expressions
     */
    ...
    /**
     * @return the weight associated with one property containing the entire name of another
     *     property
     */
     ...
     *
     * @param weight the weight associated with one property containing the entire name of another
     *     property
    ...
    /**
     * @return the weight modifier associated with a property word's edit distance based on it's
     *     nesting depth
     */
    ...
    /**
     * Set the weight modifier associated with a property word's edit distance based on it's nesting
     * depth; higher values here causes the matching to be more focused toward the final name of a
     * nested property, lower values focus on the entire name more evenly
     *
    ...
    /**
     * @return the weight associated with the number of unmatched words between two property
     *     expressions
     */
     ...
     *
     * @param weight the weight associated with the number of unmatched words between two property
     *     expressions
     * @return this instance of PropertyMatchingWeights
     */
    ...
    /**
     * @return the weight associated with the edit distance between words in two property
     *     expressions
     */
     ...
     *
     * @param weight the weight associated with the edit distance between words in two property
     *     expressions
     * @return this instance of PropertyMatchingWeights
     */
  ...
  public static class Factory extends ClassMapBuilderFactory {
     ...
     * Constructs a new Factory for ScoringClassMapBuilder instances
     *
     * @param matchingWeights the weights used to control the scorin on ScoringClassMapBuilder
    ...
    protected <A, B> ClassMapBuilder<A, B> newClassMapBuilder(
      ...
      return new ScoringClassMapBuilder<A, B>(
          aType, bType, mapperFactory, propertyResolver, defaults, matchingWeights);
    }
  }
  ...
  public static class FieldMatchScore implements Comparable<FieldMatchScore> {
    ...
    private static final List<String> IGNORED_WORDS =
        Arrays.asList("with", "this", "that", "an", "a", "of", "the");
    /*
    ...
    /**
     * Constructs a new FieldMatchScore based on the provided pair of properties, with scoring
     * modified by the provided PropertyMatchingWeights
     *
     * @param propertyA
     * @param propertyB
     * @param matchingWeights
     */
    public FieldMatchScore(
        Property propertyA, Property propertyB, PropertyMatchingWeights matchingWeights) {
      ...
      this.contains =
          propertyA.getName().contains(propertyB.getName())
    ...
    private static int getLevenshteinDistance(String s, String t) {
      ...
      for (indexOfT = 1; indexOfT <= lengthOfT; indexOfT++) {
        ...
        for (indexOfS = 1; indexOfS <= lengthOfS; indexOfS++) {
          ...
          costs[indexOfS] =
              Math.min(
                  Math.min(costs[indexOfS - 1] + 1, previousCosts[indexOfS] + 1),
                  previousCosts[indexOfS - 1] + cost);
        }
    ...
    /**
     * Splits a given property expression into arrays of lower-case words; result is returned as a
     * set of String[], which represent a property component split on word boundaries.
     *
     * @param s
     * @return
     */
    ...
    public String toString() {
      return "["
          + propertyA.getExpression()
          + ", "
          + propertyB.getExpression()
          + "] {\n"
          + "   wordMatchScore: "
          + wordMatchScore
          + "\n"
    ...
    /**
     * Compute the match score between two properties, broken up into arrays of words at each
     * property divider level.
     *
     * @param aWords
     * @param bWords
     * @return
    ...
    private void computeOverallScore() {
      ...
      if (this.commonWordCount == 0) {
        ...
        this.commonWordsScore =
            (this.matchingWeights.commonWordCount())
                * (Math.pow(2 * this.commonWordCount, 2.0)
                    * ((avgWordCount + commonWordCount) / avgWordCount));
      }
    ...
    public boolean equals(Object obj) {
      ...
      if (propertyA == null) {
        if (other.propertyA != null) return false;
      } else if (!propertyA.equals(other.propertyA)) return false;
      if (propertyB == null) {
        if (other.propertyB != null) return false;
      } else if (!propertyB.equals(other.propertyB)) return false;
      return true;
    }

    /**
     * WordPair is used to rank a match of a given set of words based on word depth and levenshtein
    ...
    private static class WordPair implements Comparable<WordPair> {
      ...
      public boolean equals(Object obj) {
        ...
        if (aWord == null) {
          if (other.aWord != null) return false;
        } else if (!aWord.equals(other.aWord)) return false;
        if (bWord == null) {
          if (other.bWord != null) return false;
        } else if (!bWord.equals(other.bWord)) return false;
