...
/**
 * ClassMapBuilderFactory should be used to construct the new instances of ClassMapBuilder needed to
 * register a mapping from one class/type to another class/type.
 */
public abstract class ClassMapBuilderFactory {
  ...
  public <A, B> ClassMapBuilderFactory chooseClassMapBuilderFactory(Type<A> aType, Type<B> bType) {
    if (appliesTo(aType, bType)) return this;
    return nextClassMapBuilderFactory == null
        ? null
   ...
   */
  protected abstract <A, B> ClassMapBuilder<A, B> newClassMapBuilder(
      Type<A> aType,
      Type<B> bType,
   ...
   * @return
   */
  private final synchronized <A, B> ClassMapBuilder<A, B> getNewClassMapBuilder(
