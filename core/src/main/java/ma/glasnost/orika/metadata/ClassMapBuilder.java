...
/**
 * ClassMapBuilder provides a fluent API which can be used to define a mapping from one class to
 * another.
 *
 * @param <A>
 * @param <B>
...
public class ClassMapBuilder<A, B> implements MappedTypePair<A, B> {
  ...
  /**
   * Note: this static member variable exists to support the deprecated static map methods; it can
   * be removed as soon as they are removed
   */
      ...
      synchronized (ClassMapBuilder.class) {
        if (defaultPropertyResolver == null || defaultPropertyResolver.get() == null) {
          defaultPropertyResolver =
              new WeakReference<PropertyResolverStrategy>(
  ...
  /**
   * Gets all of the property expressions for a given type, including all nested properties. If the
   * type of a property is not immutable and has any nested properties, it will not be included.
   * (Note that the 'class' property is explicitly excluded.)
   *
  ...
  protected Map<String, Property> getPropertyExpressions(Type<?> type) {
    ...
    LinkedHashMap<String, Property> toProcess =
        new LinkedHashMap<String, Property>(propertyResolver.getProperties(type));
        ...
        if (!propertyType.isImmutable()) {
          ...
          if (propertyType.isMap()) {
            Map<String, Property> valueProperties =
                getPropertyExpressions(propertyType.getNestedType(1));
            for (Entry<String, Property> prop : valueProperties.entrySet()) {
              Property elementProp =
                  new NestedElementProperty(entry.getValue(), prop.getValue(), propertyResolver);
              String key =
                  entry.getKey()
                      + PropertyResolver.ELEMENT_PROPERT_PREFIX
                      + prop.getKey()
            ...
            Map<String, Property> valueProperties =
                getPropertyExpressions(propertyType.getNestedType(0));
            for (Entry<String, Property> prop : valueProperties.entrySet()) {
              Property elementProp =
                  new NestedElementProperty(owningProperty, prop.getValue(), propertyResolver);
              String key =
                  entry.getKey()
                      + PropertyResolver.ELEMENT_PROPERT_PREFIX
                      + prop.getValue().getExpression()
                      + PropertyResolver.ELEMENT_PROPERT_SUFFIX;
              toProcess.put(key, elementProp);
            }
          } else if (propertyType.isArray()) {
            Map<String, Property> valueProperties =
                getPropertyExpressions(propertyType.getComponentType());
            for (Entry<String, Property> prop : valueProperties.entrySet()) {
              Property elementProp =
                  new NestedElementProperty(entry.getValue(), prop.getValue(), propertyResolver);
              String key =
  ...
  public FieldMapBuilder<A, B> fieldMap(String fieldNameA, String fieldNameB, boolean byDefault) {
    ...
    try {
      final FieldMapBuilder<A, B> fieldMapBuilder =
          new FieldMapBuilder<A, B>(
              this,
              fieldNameA,
              fieldNameB,
              byDefault,
  ...
  public FieldMapBuilder<A, B> fieldMap(Property fieldA, Property fieldB, boolean byDefault) {
    return new FieldMapBuilder<A, B>(
        this, fieldA, fieldB, byDefault, sourcesMappedOnNull, destinationsMappedOnNull);
  }
  ...
  public FieldMapBuilder<A, B> fieldMap(String fieldNameA, Property fieldB, boolean byDefault) {
    return new FieldMapBuilder<A, B>(
        this,
        resolvePropertyForA(fieldNameA),
        fieldB,
        byDefault,
        sourcesMappedOnNull,
        destinationsMappedOnNull);
  ...
  public FieldMapBuilder<A, B> fieldMap(Property fieldA, String fieldNameB, boolean byDefault) {
    return new FieldMapBuilder<A, B>(
        this,
        fieldA,
        resolvePropertyForB(fieldNameB),
        byDefault,
        sourcesMappedOnNull,
        destinationsMappedOnNull);
  }
   ...
   * @return
   */
  public FieldMapBuilder<A, B> fieldMap(
      Property.Builder fieldA, Property.Builder fieldB, boolean byDefault) {
    return new FieldMapBuilder<A, B>(
        this,
        fieldA.build((PropertyResolver) propertyResolver),
        fieldB.build((PropertyResolver) propertyResolver),
        byDefault,
        sourcesMappedOnNull,
        destinationsMappedOnNull);
  }
   ...
   */
  public FieldMapBuilder<A, B> fieldMap(
      String fieldNameA, Property.Builder fieldB, boolean byDefault) {
    return new FieldMapBuilder<A, B>(
        this,
        resolvePropertyForA(fieldNameA),
        fieldB.build((PropertyResolver) propertyResolver),
   ...
   */
  public FieldMapBuilder<A, B> fieldMap(
      Property.Builder fieldA, String fieldNameB, boolean byDefault) {
    return new FieldMapBuilder<A, B>(
        this,
        fieldA.build((PropertyResolver) propertyResolver),
        resolvePropertyForB(fieldNameB),
  ...
  /**
   * Configure this ClassMapBuilder to use an existing mapping (for parent classes) defined from
   * <code>aParentClass</code> to <code>bParentClass</code>.
   *
  ...
  public <X, Y> ClassMapBuilder<A, B> use(Type<?> aParentType, Type<?> bParentType) {
    ...
    if (!aParentType.isAssignableFrom(aType)) {
      throw new MappingException(
          aType.getSimpleName() + " is not a subclass of " + aParentType.getSimpleName());
    }

    if (!bParentType.isAssignableFrom(bType)) {
      throw new MappingException(
          bType.getSimpleName() + " is not a subclass of " + bParentType.getSimpleName());
    }
  ...
  /**
   * Configures this class-map builder to employ the default property mapping behavior to any
   * properties that have not already been mapped or excluded; if any DefaultFieldMapper instances
   * are passed, they will be used (instead of those configured on the builder) to attempt a
   * property name match if a direct match is not found.
   *
   * @param withDefaults zero or more DefaultFieldMapper instances to apply during the default
   *     mapping; if none are supplied, the configured DefaultFieldMappers for the builder (if any)
   *     should be used.
  ...
  /**
   * Produces a ClassMap instance based on the configurations defined on this ClassMapBuilder. A
   * ClassMap is used by Orika as a runtime descriptor for the details of a mapping between one type
   * and another.
   *
  ...
  /**
   * @param destinationsMappedOnNull true|false to indicate whether the destination properties of
   *     this class map's fields should be set to null (when mapping in the forward direction) if
   *     the source property's value is null
  ...
  /**
   * @param sourcesMappedOnNull true|false to indicate whether the source properties of this class
   *     map's fields should be set to null (when mapping in the reverse direction) if the
   *     destination property's value is null
   * @return this FieldMapBuilder
   */
  ...
  /**
   * @param favorsExtension true|false indicating whether the class-map defined by this builder
   *     should be considered 'abstract', meaning that it is not usable independently, but can
   *     contribute to the definition of mappers that inherit from it based on the mapped types.
   * @return
   */
  ...
  public void register() {
    if (this.mapperFactory == null) {
      throw new IllegalStateException(
          "register() is not supported from deprecated static ClassMapBuilder.map(..) instances");
    }
