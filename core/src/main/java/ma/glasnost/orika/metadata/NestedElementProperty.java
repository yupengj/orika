...
/**
 * NestedElementProperty represents a property which belongs to the type of a nested list/array/map
 * element within another type.
 *
 * @author mattdeboer
 */
public class NestedElementProperty extends Property {
   ...
   * @param resolver
   */
  public NestedElementProperty(
      Property owningProperty, Property elementProperty, PropertyResolverStrategy resolver) {
    super(
        getElementExpression(owningProperty, elementProperty),
        elementProperty.getName(),
        elementProperty.getGetter(),
        elementProperty.getSetter(),
        elementProperty.getType(),
        elementProperty.getElementType(),
  ...
  private static Property initContainer(
      Property owningProperty, String propertyExpression, PropertyResolverStrategy resolver) {
    ...
    if (parts.length > 1) {
      ...
      Property container =
          "".equals(containerExpression.toString())
              ? owningProperty
              : resolver.getProperty(owningProperty, containerExpression.toString());
      return container;
    } else {
  ...
  private static Property initElement(
      Property container, String propertyExpression, PropertyResolverStrategy resolver) {
    String[] parts = propertyExpression.replace("}", "").split("\\{");
    if (parts.length > 1) {
