...
/**
 * Type is an implementation of ParameterizedType which may be used in various mapping methods where
 * a Class instance would normally be used, in order to provide more specific details as to the
 * actual types represented by the generic template parameters in a given class.<br>
 * <br>
 * Such details are not normally available at runtime using a Class instance due to type-erasure.
 * <br>
 * <br>
 * Type essentially provides a runtime token to represent a ParameterizedType with fully-resolve
 * actual type arguments; it will contain
 *
 * @param <T>
 */
public final class Type<T> implements ParameterizedType, Comparable<Type<?>> {
  ...
  Type(
      final TypeKey key,
      final Class<?> rawType,
      final Map<String, Type<?>> typesByVariable,
  ...
  /** @return true if the given type or any of its ancestors is a parameterized type */
  ...
  private Type<?> resolveGenericAncestor(final java.lang.reflect.Type ancestor) {
    ...
    if (ancestor instanceof ParameterizedType) {
      ...
      throw new IllegalStateException(
          "super-type of "
              + this.toString()
              + " is neither Class, nor ParameterizedType, but "
              + ancestor);
    }
    return resolvedType;
  }
  ...
  public <X> Type<X> getNestedType(final int index) {
    return (Type<X>)
        ((index > -1 && actualTypeArguments.length > index) ? actualTypeArguments[index] : null);
  }

  /**
   * @return the direct super-type of this type, with type arguments resolved with respect to the
   *     actual type arguments of this type.
   */
  ...
  /**
   * @return the interfaces implemented by this type, with type arguments resolved with respect to
  ...
  /**
   * Test whether this type is assignable from the other Class; returns true if this type is not
   * parameterized and the raw type is assignable.
   *
   * @param other
   * @return
   */
  ...
  public boolean isWrapperFor(final Type<?> primitive) {
    return primitive != null
        && isPrimitiveWrapper()
        && ClassUtil.getPrimitiveType(this.rawType).equals(primitive.getRawType());
  }

  public boolean isPrimitiveFor(final Type<?> wrapper) {
    return wrapper != null
        && isPrimitive()
        && ClassUtil.getPrimitiveType(wrapper.rawType).equals(getRawType());
  }
  ...
  /**
   * Verifies whether the Type has a static valueOf method available for converting a String into an
   * instance of the type.<br>
   * Note that this method will also return true for primitive types whose corresponding wrapper
   * types have a static valueOf method.
   *
   * @return
   */
