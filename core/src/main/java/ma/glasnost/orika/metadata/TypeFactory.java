...
/**
 * TypeFactory contains various methods for obtaining a Type instance to represent various type
 * situations.
 */
public abstract class TypeFactory {

  /**
   * Use a weak-valued concurrent map to avoid keeping static references to Types (classes) which
   * may belong to descendant class-loaders
   */
  ...
  public static <E> Type<E> valueOf(final Class<E> rawType) {
    if (rawType == null) {
      return null;
        ...
        && rawType.getGenericSuperclass() instanceof ParameterizedType) {
  ...
  /**
   * Constructs a nested type from a string description of that type; allows for package names to be
   * omitted for 'java.lang' and 'java.util' classes.
   *
  ...
  /**
   * Resolve the Type value of the given raw Class type, filling the type parameters with the
   * provided actual type arguments
   *
   * @param rawType
   * @param actualTypeArguments
   * @return the resolved Type instance
   */
  public static <E> Type<E> valueOf(
      final Class<E> rawType, final java.lang.reflect.Type... actualTypeArguments) {
    if (rawType == null) {
      return null;
    } else {
  ...
  /**
   * This method declaration helps to shortcut the other methods for ParameterizedType which it
   * extends; we just return it.
   *
   * @param type
   * @return the resolved Type instance
   */
  ...
  /**
   * Return the Type for the given ParameterizedType, resolving actual type arguments where
   * possible.
   *
   * @param type
   * @return the resolved Type instance
  ...
  public static <T> Type<T> valueOf(final WildcardType var) {
    ...
    Set<Type<?>> bounds =
        new HashSet<Type<?>>(var.getUpperBounds().length + var.getLowerBounds().length);
  ...
  /**
   * Return the Type for the given java.lang.reflect.Type, either for a ParameterizedType or a Class
   * instance
   *
   * @param type
   * @return the resolved Type instance
  ...
  /**
   * Resolve the Type for the given ParameterizedType, using the provided referenceType to resolve
   * any unresolved actual type arguments.
   *
   * @param type
   * @param referenceType
   * @return the resolved Type instance
   */
  @SuppressWarnings("unchecked")
  public static <T> Type<T> resolveValueOf(
      final ParameterizedType type, final Type<?> referenceType) {
    if (type == null) {
      return null;
    } else {
      java.lang.reflect.Type[] actualTypeArguments =
          TypeUtil.resolveActualTypeArguments(type, referenceType);
      return intern(
          (Class<T>) type.getRawType(), actualTypeArguments, new HashSet<java.lang.reflect.Type>());
    }
  }

  /**
   * Resolve the Type for the given Class, using the provided referenceType to resolve the actual
   * type arguments.
   *
   * @param type
   * @param referenceType
  ...
  public static <T> Type<T> resolveValueOf(final Class<T> type, final Type<?> referenceType) {
    if (type == null) {
      ...
      if (type.getTypeParameters() != null && type.getTypeParameters().length > 0) {
        java.lang.reflect.Type[] actualTypeArguments =
            TypeUtil.resolveActualTypeArguments(
                type.getTypeParameters(), type.getTypeParameters(), referenceType);
  ...
  /**
   * Gets the name for a type, using the Simple name when the type does not match the associated
   * type, but using fully-qualified name when the associated type has a matching simple name, but
   * is not the same.
   *
   * @param type the type for which to return the name
   * @param associatedType the associated type, used to decide the level of detail required in the
   *     returned name
   * @return
   */
  ...
  /**
   * Return the Type for the given java.lang.reflect.Type, limiting the recursive depth on any type
   * already contained in recursiveBounds.
   *
   * @param type
   ...
   */
  @SuppressWarnings("unchecked")
  static <T> Type<T> limitedValueOf(
      final java.lang.reflect.Type type, final Set<java.lang.reflect.Type> recursiveBounds) {
    if (type instanceof Type) {
      return (Type<T>) type;
    } else if (type instanceof ParameterizedType) {
  ...
  @SuppressWarnings("unchecked")
  static <T> Type<T> limitedValueOf(
      final TypeVariable<?> var, final Set<java.lang.reflect.Type> recursiveBounds) {
   ...
   */
  @SuppressWarnings("unchecked")
  private static <T> Type<T> limitedValueOf(
      final WildcardType var, final Set<java.lang.reflect.Type> recursiveBounds) {
  ...
  /**
   * Return the Type for the given ParameterizedType, resolving actual type arguments where
   * possible; uses recursiveBounds to limit the recursion.
   *
   * @param type
   * @param recursiveBounds
   * @return the resolved Type instance
  ...
  @SuppressWarnings("unchecked")
  private static <T> Type<T> limitedValueOf(
      final ParameterizedType type, final Set<java.lang.reflect.Type> recursiveBounds) {
    return limitedValueOf(
        (Class<T>) type.getRawType(), recursiveBounds, type.getActualTypeArguments());
  }
   ...
   * @return the resolved Type instance
   */
  private static <E> Type<E> limitedValueOf(
      final Class<E> rawType,
      Set<java.lang.reflect.Type> recursiveBounds,
  ...
  /**
   * Store the combination of rawType and type arguments as a Type within the type cache.<br>
   * Use the existing type if already available; we try to enforce that Type should be immutable.
   *
   ...
   */
  @SuppressWarnings("unchecked")
  private static <T> Type<T> intern(
      final Class<T> rawType,
      final java.lang.reflect.Type[] typeArguments,
      final Set<java.lang.reflect.Type> recursiveBounds) {

    Type<?>[] convertedArguments =
        TypeUtil.convertTypeArguments(rawType, typeArguments, recursiveBounds);
