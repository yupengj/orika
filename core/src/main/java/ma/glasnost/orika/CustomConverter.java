...
/**
 * CustomConverterBase provides a utility base for creating customized converters, which determines
 * type parameters automatically. <br>
 * <br>
...
public abstract class CustomConverter<S, D> implements ma.glasnost.orika.Converter<S, D> {
      ...
      */
      ...
      if (actualTypeArguments.length == 2) {
        ...
        sourceType =
            (Type<S>)
                TypeFactory.valueOf(
                    ClassHelper.findParameterClass(0, getClass(), CustomConverter.class));
        destinationType =
            (Type<D>)
                TypeFactory.valueOf(
                    ClassHelper.findParameterClass(1, getClass(), CustomConverter.class));
      }
    } else {
      throw new IllegalStateException(
          "When you subclass CustomConverter S and D type-parameters are required.");
    }
    /*
  ...
  public String toString() {
    String subClass =
        getClass().equals(CustomConverter.class) || getClass().isAnonymousClass()
            ? ""
