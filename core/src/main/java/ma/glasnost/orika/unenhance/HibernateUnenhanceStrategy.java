...
/**
 * HibernateUnenhanceStrategy is used to unwrapped objects from their Hibernate-generated proxy,
 * which may have been created to match methods of a super-type (missing some of the details
 * important in a child class mapping).
 */
public class HibernateUnenhanceStrategy implements UnenhanceStrategy {
  ...
  public HibernateUnenhanceStrategy() {
    try {
      ...
      Class<?> hibernate =
          Class.forName(HIBERNATE_CLASS, false, Thread.currentThread().getContextClassLoader());
      getHibernateClass = hibernate.getMethod(HIBERNATE__GET_CLASS, Object.class);

      hibernateProxy =
          Class.forName(
              HIBERNATE_PROXY_CLASS, false, Thread.currentThread().getContextClassLoader());
      getHibernateLazyInitializer = hibernateProxy.getMethod(HIBERNATE_PROXY__GET_LAZY_INITIALIZER);

      Class<?> hibernateLazyInitializer =
          Class.forName(
              LAZY_INITIALIZER_CLASS, false, Thread.currentThread().getContextClassLoader());
  ...
  private static void hibernateInaccessible(Exception e) {
    throw new ExceptionInInitializerError(
        "One of "
            + HIBERNATE_CLASS
            + "#"
            + HIBERNATE__GET_CLASS
            + "(), "
            + HIBERNATE_PROXY_CLASS
            + "#"
            + HIBERNATE_PROXY__GET_LAZY_INITIALIZER
            + "() , or "
            + LAZY_INITIALIZER_CLASS
            + "#"
            + LAZY_INITIALIZER__GET_IMPLEMENTATION
            + "() required by "
            + HibernateUnenhanceStrategy.class.getCanonicalName()
            + " is not accessible"
            + e);
  }
  ...
  private static void hibernateGetLazyInitUnavailable(Exception e) {
    LOGGER.warn(
        LAZY_INITIALIZER_CLASS + "#" + LAZY_INITIALIZER__GET_IMPLEMENTATION + " is not available; ",
        e);
  }
