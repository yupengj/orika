...
public abstract class BuiltinConverters {
  ...
  /**
   * Registers a common set of built-in converters which can handle many common conversion
   * situations.<br>
   * Specifically, this includes:
   *
   * <ul>
   *   <li>ConstructorConverter: converts from the source type to destination type if there is a
   *       constructor available on the destination type which takes the source as a single
   *       argument.
   *   <li>FromStringConverter: able to convert from a String to enum, primitive, or primitive
   *       wrapper.
   *   <li>ToStringconverter: able to convert any type to String
   *   <li>DateAndTimeConverters: convert between common date/time representations
   *       <ul>
  ...
  public static void register(ConverterFactory converterFactory) {
    ...
    converterFactory.registerConverter(
        new DateAndTimeConverters.DateToXmlGregorianCalendarConverter());
    converterFactory.registerConverter(new DateAndTimeConverters.DateToTimeConverter());
    converterFactory.registerConverter(
        new DateAndTimeConverters.CalendarToXmlGregorianCalendarConverter());
    converterFactory.registerConverter(
        new DateAndTimeConverters.XmlGregorianCalendarToTimestampConverter());
    converterFactory.registerConverter(
        new DateAndTimeConverters.XmlGregorianCalendarToSqlDateConverter());
    converterFactory.registerConverter(
        new DateAndTimeConverters.XmlGregorianCalendarToTimeConverter());
    converterFactory.registerConverter(
        new DateAndTimeConverters.LongToXmlGregorianCalendarConverter());
  ...
  static <T> Converter<T, T> makeSimpleConverter(Class<T> cls, SimpleFnConverter<T> fn) {
    ...
    return new BaseConverter<>(type, type) {
      ...
      @Override public boolean canConvert(Type<?> sourceType, Type<?> destinationType) {
