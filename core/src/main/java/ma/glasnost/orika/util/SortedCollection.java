...
/**
 * A simple sorted collection implementation that allows for duplicates; new items are inserted
 * based on their comparison to existing items; if a new item is found to be less than any item in
 * the list, it is inserted before that item, else it is inserted at the end. <br>
 * <br>
 * The collection is backed by a ConcurrentSkipListMap, with the keys defined as Double, allowing
 * for inserts of items at a specific location with respect to the other items in the map.
 *
 * <p>This class is thread-safe, with the same caveats for ConcurrentSkipListMap.
 *
    ...
    try {
      ...
      if (mustAdd(value)) {
        ...
        for (Node<V> from : nodes) {
          final OrderingRelation order = ordering.order(from.getValue(), newNode.getValue());
  ...
  private List<V> getSortedItems() {
    if (sortedItems == null) {
      try {
        ...
        if (sortedItems == null) sortedItems = TopologicalSorter.sort(nodes);
      } finally {
