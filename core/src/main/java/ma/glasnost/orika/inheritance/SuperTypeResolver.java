...
public final class SuperTypeResolver {
  ...
  public static <T> Type<T> getSuperType(
      final Type<?> enhancedClass, final SuperTypeResolverStrategy strategy) {
  ...
  private static Type<?> tryFirstLookupOption(
      final Type<?> theClass, final SuperTypeResolverStrategy strategy) {
  ...
  private static Type<?> trySecondLookupOption(
      final Type<?> theClass, final SuperTypeResolverStrategy strategy) {
    if (strategy.shouldPreferClassOverInterface()) {
      return lookupMappedInterface(theClass, strategy);
    } else {
  ...
  private static Type<?> lookupMappedSuperType(
      final Type<?> type, final SuperTypeResolverStrategy strategy) {
    ...
    while (mappedType == null
        && targetType != null
        && !targetType.getRawType().equals(Object.class)) {
  ...
  private static Type<?> lookupMappedInterface(
      final Type<?> type, final SuperTypeResolverStrategy strategy) {
