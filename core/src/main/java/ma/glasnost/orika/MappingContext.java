...
/**
 * MappingContext provides storage for information shared among the various mapping objects for a
 * given mapping request.
 */
public class MappingContext {
  ...
  public MappingContext(Map<Object, Object> globalProperties) {
    ...
    Boolean capture =
        globalProperties != null
            ? (Boolean) globalProperties.get(Properties.CAPTURE_FIELD_CONTEXT)
            : null;
    this.capturesFieldContext = capture == null || capture;
  }

  /**
   * Sets whether this MappingContext needs to guard against cycles when mapping the current object
   * graph; specifying <code>false</code> when applicable can lend improved performance.
   *
   * @param containsCycle
   */
  ...
  /**
   * Searches for a concrete class that has been registered for the given abstract class or
   * interface within this mapping session.
   *
   * @param sourceType
   * @param destinationType
   * @return a concrete class that has been registered for the given abstract class or interface
   *     within this mapping session, if any
   */
  ...
  /**
   * Registers a concrete class to be used for the given abstract class or interface within this
   * mapping session only.
   *
   * @param subjectClass
   * @param concreteClass
   */
  ...
  /**
   * Caches an object instance which has been mapped for a particular source instance and
   * destination type in this mapping context; this will later be referenced in avoiding infinite
   * recursion mapping the same object.
   *
   * @param source
   * @param destinationType
   * @param destination
  ...
  public <S, D> void cacheMappedObject(S source, Type<Object> destinationType, D destination) {
    if (containsCycle) {
      Map<Object, Object> localCache =
          (Map<Object, Object>) typeCache.get(destinationType.getUniqueIndex());
      if (localCache == null) {
  ...
  /**
   * Looks for an object which has already been mapped for the source and destination type in this
   * context.
   *
   * @param source
   * @param destinationType
   * @return the mapped object, or null if none exists for the source instance and destination type
   */
  @SuppressWarnings("unchecked")
  ...
  public void registerMapperGeneration(ClassMap<?, ?> classMap) {
    ...
    Map<MapperKey, ClassMap<?, ?>> list =
        mappersSeen.isEmpty() ? null : this.mappersSeen.get(depth - 1);
    if (list == null) {
  ...
  public ClassMap<?, ?> getMapperGeneration(MapperKey mapperKey) {
    ...
    Map<MapperKey, ClassMap<?, ?>> map =
        (mappersSeen == null || mappersSeen.isEmpty()) ? null : this.mappersSeen.get(depth - 1);
    if (map != null) {
   ...
   *
   * @deprecated This variant exists for backwards compatibility only; if overriding, override
   ...
   * @deprecated This variant exists for backwards compatibility only; if overriding, override
   *     {@link #beginMapping(Type, Object, Type, String, Object)} instead.
   */
  @Deprecated
  ...
  /**
   * @return the qualified property expression describing the source field currently being mapped
   */
  ...
  /**
   * @return an array of expressions describing the path to the source field being currently mapped;
   *     each array element contains the expression value of the field as referenced within the
  ...
  /**
   * @return an array of the source object values on the current stack, where the last element of
   *     the array is the value of the source field being currently mapped
   */
  ...
  /**
   * @return an array of types representing the path to the type of the source field being currently
   *     mapped; this represents each type in the chain of mappers called to map the current field.
   */
  ...
  /**
   * @return the qualified property expression describing the destination field currently being
   *     mapped
   */
  ...
  /**
   * @return an array of the destination object values on the current stack, where the last element
   *     of the array is the value of the destination field being currently mapped
   */
  ...
  /**
   * @return an array of types representing the path to the type of the destination field being
   *     currently mapped; this represents each type in the chain of mappers called to map the
   *     current field.
   */
  ...
  /**
   * Get a property set on the current mapping context; individual properties set on this context
   * instance are checked first, followed by global properties.
   *
   * @param key
