...
public class MultiThreadedTestCase {
  ...
  /**
   * Allows us to run methods concurrently by marking with <code>@Concurrent</code>; note that in
   * the current implementation, such methods will have the <code>@Before</code> and <code>@After
   * </code> methods also invoked concurrently.
   */
  ...
  /**
   * Verifies that multiple threads requesting the type for the same set of classes receive the same
   * set of values over a large number of classes.
   */
  @Test
  @Concurrent(25)
    ...
    for (Class<?> aClass : classes) {
      ...
      try {
        ...
        throw new RuntimeException(
            "while trying to evaluate valueOf(" + aClass.getCanonicalName() + ")", e);
      }
      if (aType == null) {
   ...
   * garbage collector before OOME is thrown, we allocate dummy bytes until we reach OOME. TODO:
   * this doesn't work on IBM Jdk, as it propagates the OOME to all threads, including the one
   * running our test!
   */
      ...
      synchronized (this) {
        ...
        try {
          for (int i = 0; i < Integer.MAX_VALUE; ++i) {
            int available =
                (int) Math.min((long) Integer.MAX_VALUE, Runtime.getRuntime().maxMemory());
