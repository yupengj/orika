...
public class TypeFactoryTestCase {
  ...
  public void createTypeFromMultipleNestedClass() {
    ...
    Assert.assertEquals(
        Map.class, type.getNestedType(0).getNestedType(1).getNestedType(0).getRawType());
    Assert.assertEquals(
        String.class,
  ...
  public void createTypeFromClassHirarchy() {
    ...
    java.lang.reflect.Type firstArgType =
        type.findInterface(interfaceType).getActualTypeArguments()[0];
    Type firstType = TypeFactory.valueOf(firstArgType);
    Assert.assertEquals(String.class, firstType.getRawType());

    java.lang.reflect.Type secondArgType =
        type.findInterface(interfaceType).getActualTypeArguments()[1];
  ...
  public void testRefineBoundsFail() throws Exception {
    ...
    thrown.expectMessage(
        containsString("Long")); // it can be "Long and String" or "String and Long"
  ...
  @SuppressWarnings("rawtypes")
  public void testRefineBoundsSuccess(Class<?> expetedClass, Class<?>... boundsClass)
      throws Exception {
  ...
  @SuppressWarnings("unused")
  public static class MyObjectWithMultibleBound<
      T extends Object & Collection<String> & Set<String>> {
    // test Class
  }
}
