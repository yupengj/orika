...
public class FilterTestCase {
  ...
  public static class SecurityFilter extends NullFilter<Object, Object> {
    ...
    public <D> D filterDestination(
      ...
      if ("creditCardNumber".equals(sourceName)) {
        ...
        destinationValue =
            (D)
                (MASK.substring(0, cardMask.length() - 4)
                    + cardMask.substring(cardMask.length() - 4));
      }
  ...
  private static class CostFilter extends NullFilter<Number, Number> {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public <S extends Number> S filterSource(
        S sourceValue,
        final Type<S> sourceType,
        final String sourceName,
        final Type<?> destType,
  ...
  /**
   * OverwriteProtectionFilter is a filter that only allows a mapping to take place if the
  ...
  private static class InfoFilter extends NullFilter<Map<?, ?>, List<?>> {
    @Override
    public <S extends Map<?, ?>, D extends List<?>> boolean shouldMap(
        final Type<S> sourceType,
        final String sourceName,
        final S source,
        final Type<D> destType,
