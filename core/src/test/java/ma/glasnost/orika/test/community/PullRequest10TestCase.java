 ...
 *
 * @see <a
 *     href="https://github.com/orika-mapper/orika/pull/10">https://github.com/orika-mapper/orika</a>
 */
public class PullRequest10TestCase {
  ...
  public void testClassInheritanceSorting() {
    ...
    for (Type<?> x : types) {
      for (Type<?> y : types) {
        assertThat(
            "sgn(x.compareTo(y)) == -sgn(y.compareTo(x)) for x="
                + x.getSimpleName()
                + ", y="
                + y.getSimpleName(),
            sgn(x.compareTo(y)),
            is(-sgn(y.compareTo(x))));
