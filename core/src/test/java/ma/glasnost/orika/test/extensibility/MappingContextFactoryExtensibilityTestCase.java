...
public class MappingContextFactoryExtensibilityTestCase {
  ...
  public void testMappingContext_withCustomMapper() {
    ...
    final MapperFactory factory =
        new DefaultMapperFactory.Builder().mappingContextFactory(contextFactory).build();

    factory
        .classMap(Person.class, Person.class)
        .customize(
            new CustomMapper<Person, Person>() {
              @Override
              public void mapAtoB(Person a, Person b, MappingContext context) {
                b.setName(a.getName());
                b.setDependsOn(
                    factory.getMapperFacade().map(a.getDependsOn(), Person.class, context));
              }
    ...
    factory
        .classMap(Woman.class, Woman.class)
        .use(Person.class, Person.class)
        .byDefault()
        .register();
  ...
  public void testMappingContext_withBoundMapperFacade() {
    ...
    factory
        .getConverterFactory()
        .registerConverter(
            new CustomConverter<Person, Person>() {

              public Person convert(
                  Person source, Type<? extends Person> destType, MappingContext mappingContext) {
                final BoundMapperFacade<Person, Person> boundMapperFacadePerson =
                    factory.getMapperFacade(Person.class, Person.class);
                final BoundMapperFacade<Child, Child> boundMapperFacadeChild =
                    factory.getMapperFacade(Child.class, Child.class);
                Person destination = boundMapperFacadePerson.newObject(source, mappingContext);
                destination.setName(source.getName());
                destination.setDependsOn(
                    boundMapperFacadePerson.map(source.getDependsOn(), mappingContext));
        ...
        .registerConverter(
            new CustomConverter<Person, Person>() {
              ...
              public Person convert(
                  Person source, Type<? extends Person> destType, MappingContext mappingContext) {
                Person destination =
                    factory.getMapperFacade().newObject(source, destinationType, mappingContext);
                destination.setName(source.getName());
                destination.setDependsOn(
                    factory
                        .getMapperFacade()
                        .map(source.getDependsOn(), Person.class, mappingContext));
                if (source instanceof Parent && destination instanceof Parent) {
                  ...
                  factory
                      .getMapperFacade()
                      .mapAsCollection(
                          srcParent.getChildren(),
                          destParent.getChildren(),
                          childType,
                          childType,
                          mappingContext);
                }
  ...
  private void assertComplexPerson(Person mapped) {
    ...
    Assert.assertTrue(
        "Maxi must be a Man but was: " + mapped.getClass().getSimpleName(), mapped instanceof Man);
    Assert.assertTrue(
        "Susi must be a Woman but was: " + mapped.getDependsOn().getClass().getSimpleName(),
        mapped.getDependsOn() instanceof Woman);
    Assert.assertTrue(
        "Hansi must be a Man but was: "
            + mapped.getDependsOn().getDependsOn().getClass().getSimpleName(),
  ...
  public static class MyMappingContext extends MappingContext {
    ...
    public static class Factory implements MappingContextFactory {
      private ConcurrentHashMap<Object, Object> globalProperties =
          new ConcurrentHashMap<Object, Object>();
      private ThreadLocal<MappingContext> threadLocalMappingContext =
          new ThreadLocal<MappingContext>();

      @Override
      public MappingContext getContext() {
        if (threadLocalMappingContext.get() != null) {
          throw new IllegalStateException(
              "During this Thread only one MappingContext is allowed to be created.");
        }
