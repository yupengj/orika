...
public class MultipleMapperWrapperTest {
   ...
   * @see Issue24TestCase
   * @see <a
   *     href="https://code.google.com/archive/p/orika/issues/24">https://code.google.com/archive/p/orika/</a>
   */
  @Test
  public void testFindMapper_withSimpleHirarchy() {
    ...
    final MultipleMapperWrapper multipleMapper =
        createMultipleMapperWrapper(
    ...
    assertThat(
        expectedException.getMessage(), containsString("No matching Mapper found for A <-> B\n"));
    ...
    assertThat(
        ...
        containsString("No matching Mapper found for A <-> BSub2\n"));
    assertThat(expectedException.getMessage(), containsString("Existing Mapper: A <-> BSub\n"));
  }
   ...
   * @see <a
   *     href="https://code.google.com/archive/p/orika/issues/121">https://code.google.com/archive/p/orika/</a>
   */
  @Test
  public void testFindMapper_withGenericList() {
    ...
    multipleMapper.mapAtoB(
        Collections.unmodifiableList(new ArrayList<A>()), new ArrayList<BSub>(), null);
    ...
    assertThat(
        expectedException.getMessage(),
        containsString("No matching Mapper found for HashSet<Object> <-> ArrayList<Object>\n"));
    assertThat(
        expectedException.getMessage(),
        containsString("Existing Mapper: List<A> <-> List<BSub>\n"));
    ...
    assertThat(
        expectedException.getMessage(),
        containsString("No matching Mapper found for ArrayList<Object> <-> HashSet<Object>\n"));
    assertThat(
        expectedException.getMessage(),
        containsString("Existing Mapper: List<A> <-> List<BSub>\n"));
  }
  ...
  private MultipleMapperWrapper createMultipleMapperWrapper(
      Type<?> aTypeMapper, Type<?> bTypeMapper, Type<?> aTypeWrapper, Type<?> bTypeWrapper) {
    ...
    List<Mapper<Object, Object>> mappers =
        Collections.singletonList((Mapper<Object, Object>) generatedMapper);
    @SuppressWarnings("unchecked")
    MultipleMapperWrapper multipleMapper =
        new MultipleMapperWrapper(
            (Type<Object>) aTypeWrapper, (Type<Object>) bTypeWrapper, mappers);
