 ...
 *
 * @see <a
 *     href="https://github.com/orika-mapper/orika/issues/167">https://github.com/orika-mapper/orika/issues</a>
 */
public class Issue167TestCase {
  ...
  public void testIssue167() throws Exception {
    ...
    factory.classMap(ChildInterface.class, ChildInterface.class).byDefault().register();
    ...
    assertThat(
        newChild.getSet().iterator().next(), is(not(foo))); // must be a new mapped instance...
    assertThat(
        newChild.getSet().iterator().next().getName(), is("test-Foo")); // with the same value.
  }
  ...
  public void testCaseSimplified_withPropertyResolverResolvePropertyType() throws Exception {
    ...
    Type<?> resolvedPropertyType =
        propRes.resolvePropertyType(readMethod, rawType, owningType, referenceType);
