...
public class PersonMappingTestCase {
  ...
  /**
   * RUn 3 maps, rely on engine to map per the sub type. Only one map appears to be retained in
   * memory. The cache must key on the source type. This means you cannot have multiple maps with
  ...
  public void testMultiMap() throws Exception {
    ...
    mapperFactory
        ...
        .field(
            "personNameA",
            "persons:{getPersons()|setPersons(%s)|type=List<"
                + SuffixPerson.class.getCanonicalName()
                + ">}[0]") // suffix
  ...
  /**
   * Uses one map only. This has a problem picking up the sub class contained in the list. When the
   * syntax on the EL seems correct, get a stackoverflow exception.
   *
   * @throws Exception
   */
  @Test
